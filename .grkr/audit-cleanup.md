...
## t_5131a4ee (2026-06-24 cron) — append auth.lock removal (orchestrator)

**Task:** hygiene: append audit-cleanup.md auth.lock removal (cron 2026-06-24)

| Signal | Value |
|--------|-------|
| HEAD | `1657e59` on `v2` (workspace `/Users/claw/work/grkr-v2-cron`) |
| Worker scope | `.grkr/audit-cleanup.md` append only (orchestrator already ran `rm -f`) |

**Orchestrator action (pre-worker):** removed empty stale `~/.hermes/auth.lock` (0 bytes, lsof unheld, not held).

**Post-append verification (worker):** `gateway.lock` 155B Jun 19 — **keep** (live gateway); `kanban.db.init.lock` 0B — **keep**; no `rm -rf`; `.grkr/locks/` empty; gleam test suite unchanged.

**Metadata:** `{locks_removed: ["auth.lock"], changed_files: [".grkr/audit-cleanup.md"]}`

# Generated by kanban task t_5131a4ee

## Kanban hygiene — PR #79 duplicate e2e/review todos (2026-06-24)

**Task:** t_ec9814a5 (cron orchestrator)

**Kept (canonical gate chain):**
- `t_04dfcd30` — gate: PR #79 merge readiness (blocked)
- `t_ae615575` — canonical cron e2e @ v2 HEAD post-hygiene (running; grkr-orchestrator-20260624-e2e-canonical)
- `t_3fd558a8` — review checklist post canonical e2e (todo, parent `t_ae615575`)

**Archived (superseded @ old SHAs / duplicate hygiene):**
`t_e4c37e96`, `t_2251b4cf`, `t_a8448bf1`, `t_e97a478f`, `t_ff36b74c`, `t_daca8e68`, `t_dcc19995`, `t_e3c613d5`

**Action:** `hermes kanban archive` on the eight ids above; no repo code changes beyond this audit append.

# Generated by kanban task t_b963fc2a

## t_b963fc2a (2026-06-25) — gitignore ephemeral root kanban/Hermes scratch

**Task:** hygiene: gitignore ephemeral `.worktrees/`, `undefined/`, `.tmp_*`, `.hermes-kanban-*.sh`

**Observed untracked (not deleted; audit-only):**
- `.worktrees/t_60ef75dc/` — stale kanban worktree registration (see prior audit worktree table)
- `undefined/.grkr/` — mistaken cwd artifact (logs/state under bogus top-level dir)
- `.tmp_archive_ids.py`, `.tmp_kanban_filter.py` — one-off kanban cron helpers
- `.hermes-kanban-orchestrator-create.sh`, `.hermes-kanban-robot-check.sh` — local Hermes dispatch scripts

**Repo changes:** `.gitignore` entries for the patterns above; README runtime-state bullet updated. No deletes.

**Metadata:** `{changed_files: [".gitignore", "README.md", ".grkr/audit-cleanup.md"]}`

# Generated by kanban task t_de693827

## t_de693827 (2026-06-25) — clear unheld empty lock files (cron)

**Removed:** `~/.hermes/auth.lock` (0B, lsof empty), `.grkr/locks/main.lock` (0B, lsof empty). No other deletes.


## t_c3986548 (2026-07-02) — prune stale worktrees + lock audit + docs HEAD sync

**HEAD before/after work:** `6a34d26` on `v2` (= `origin/v2` at start).

### Worktrees pruned
| Path | Branch | Tip | Reason |
|------|--------|-----|--------|
| `/Users/claw/work/grkr-v2-cron/.worktrees/t_60ef75dc` | `wt/cron-fs-mjs-20260606` | `7b95709` | Abandoned Jun 6; unique commit only `f != null` guard already present on live `src/grkr/supervisor/fs.mjs` |
| `/Users/claw/.hermes/.worktrees/t_abec58cb` | `wt/refusal-flow-cli-tests-20260606` | `531762f` | Abandoned Jun 6; tip ancestor of `origin/v2`; untracked refusal smoke stubs already on v2 |

Commands: `git worktree remove --force <path>` ×2; `git worktree prune -v` (noop after remove). Post-list: only main worktree.

**Not deleted:** local branch names `wt/cron-fs-mjs-20260606`, `wt/refusal-flow-cli-tests-20260606` (no attached worktree; force branch delete approval-gated — harmless orphans).

### Locks
| Path | Action |
|------|--------|
| `.grkr/locks/main.lock` (0B empty flock placeholder) | **KEEP** (no live holder; normal flock file) |
| `~/.hermes/gateway.lock`, `auth.lock`, `kanban.db.*.lock` | **out of scope** — not touched |

### gitignore
Added: `.grkr/kanban-cron-orchestrator-body*.md`, `.grkr/*.log`, `.grkr/tmp-*.sh`. Did **not** mass-delete historical body files.

### Docs
README + `docs/gleam-migration.md` + `spec/parts/39` status SHA `18665c3` → `6a34d26` (295 tests unchanged).

**Metadata:** `{pruned_worktrees: 2, locks_touched: 0, locks_kept: ["main.lock"], changed_files: [".gitignore","README.md","docs/gleam-migration.md","spec/parts/39-recommended-implementation-order.md",".grkr/audit-cleanup.md"]}`

# Generated by kanban task t_c3986548


## t_786372b0 (2026-07-02) — safe stale lock audit + worktree prune (cron, no rm -rf)

**HEAD:** `b870c69` on `v2` (= `origin/v2`).

### Locks audited
| Path | Size | lsof | Action |
|------|------|------|--------|
| `~/.hermes/auth.lock` | 0B | none | **REMOVED** (stale empty, unheld) |
| `~/.hermes/kanban.db.dispatch.lock` | 0B | none | **REMOVED** (stale empty, unheld) |
| `~/.hermes/kanban.db.init.lock` | 0B | none | **REMOVED** (stale empty, unheld; dated Jun 27) |
| `~/.hermes/gateway.lock` | 164B (`pid:11328`, kind hermes-gateway) | held by `python3.1` PID 11328 | **KEPT** (live process) |
| `.grkr/locks/` | empty dir | n/a | nothing to remove (main.lock already gone from prior hygiene) |

### Worktrees
`git worktree prune -v` — noop (only main worktree `/Users/claw/work/grkr-v2-cron` on `v2` @ `b870c69`). No abandoned worktrees present after t_c3986548.

### Not done (out of scope)
No `rm -rf` of worktrees/large trees; no config.sh edits; no Linear work; no force-push.

**Metadata:** `{locks_removed: ["~/.hermes/auth.lock","~/.hermes/kanban.db.dispatch.lock","~/.hermes/kanban.db.init.lock"], locks_kept: ["~/.hermes/gateway.lock"], worktrees_pruned: 0, commit_sha: pending}`

# Generated by kanban task t_786372b0


## t_6859d4bc (2026-07-02 later) — safe stale lock audit + worktree prune (cron, no rm -rf)

**HEAD:** `4a50bd9` on `v2` (= `origin/v2`).

### Locks audited
| Path | Size | lsof | Action |
|------|------|------|--------|
| `~/.hermes/auth.lock` | 0B (mtime 14:35) | none | **REMOVED** (stale empty, unheld; reappeared after t_786372b0) |
| `~/.hermes/kanban.db.dispatch.lock` | 0B (mtime 08:35) | none | **REMOVED** (stale empty, unheld) |
| `~/.hermes/kanban.db.init.lock` | 0B (mtime 12:11) | none | **REMOVED** (stale empty, unheld) |
| `~/.hermes/gateway.lock` | 164B (`pid:56697`, kind hermes-gateway, `--replace`) | held by `python3.1` PID 56697 | **KEPT** (live process) |
| `.grkr/locks/` | empty dir | n/a | nothing to remove |

### Worktrees
`git worktree prune -v` — noop (only main worktree `/Users/claw/work/grkr-v2-cron` on `v2` @ `4a50bd9`).

### Documented only (not deleted — needs human approval)
| Path | Size | Notes |
|------|------|-------|
| `~/.hermes/kanban/workspaces/t_c55adf10` | 16M | May 26 snapshot of grkr checkout (build locks only per orchestrator note); **do not GC without approval** |
| `~/.hermes/kanban/workspaces/t_1bbce0f3` | 4.0K | May 26 empty-ish dispatch artifact |
| `~/.hermes/kanban/workspaces/t_73669aac` | 0B | Jun 6 empty dispatch artifact |

### Not done (out of scope)
No `rm -rf` of worktrees/kanban workspaces/large trees; no config.sh edits; no Linear work; no force-push.

**Metadata:** `{locks_removed: ["~/.hermes/auth.lock","~/.hermes/kanban.db.dispatch.lock","~/.hermes/kanban.db.init.lock"], locks_kept: ["~/.hermes/gateway.lock"], worktrees_pruned: 0, workspaces_noted: ["t_c55adf10","t_1bbce0f3","t_73669aac"], commit_sha: pending}`

# Generated by kanban task t_6859d4bc


## t_b6e07a3b (2026-07-02 cron) — remove stale empty ~/.hermes/auth.lock + audit note

**HEAD (pre):** `454a65a` on `v2` (= `origin/v2`).

### Locks audited
| Path | Size | lsof | Action |
|------|------|------|--------|
| `~/.hermes/auth.lock` | 0B (mtime 2026-07-02 19:00) | none (lsof exit 1) | **REMOVED** (stale empty, unheld; reappeared after t_6859d4bc) |
| `~/.hermes/gateway.lock` | 164B (mtime 12:23) | live hermes-gateway | **KEPT** (live process) |
| `~/.hermes/dispatch.lock` | missing | n/a | nothing |
| `~/.hermes/init.lock` | missing | n/a | nothing |
| `.grkr/locks/` | empty / unchanged | n/a | nothing to remove |

### Worktrees
`git worktree prune -v` — noop (only main worktree `/Users/claw/work/grkr-v2-cron` on `v2` @ `454a65a`).

### Not done (out of scope)
No `rm -rf` of worktrees/kanban workspaces/large trees; no gateway/dispatch/init lock removal; no config.sh edits; no Linear work; no force-push.

**Metadata:** `{locks_removed: ["~/.hermes/auth.lock"], locks_kept: ["~/.hermes/gateway.lock"], worktrees_pruned: 0, commit_sha: pending}`

# Generated by kanban task t_b6e07a3b


## t_ee0b9cb6 (2026-07-03 cron) — lock/worktree audit + optional docs tip sync @ bd2bc96

**HEAD:** `bd2bc96` on `v2` (= `origin/v2`). Branch clean of tracked changes; one untracked orchestrator id file (left untracked).

### Locks audited
| Path | Size | lsof | Action |
|------|------|------|--------|
| `~/.hermes/auth.lock` | 0B (mtime 2026-07-02 21:33) | none | **REMOVED** (stale empty, unheld; reappeared after t_b6e07a3b) |
| `~/.hermes/kanban.db.dispatch.lock` | 0B (mtime 2026-07-02 14:36) | none | **REMOVED** (stale empty, unheld; reappeared after t_6859d4bc) |
| `~/.hermes/kanban.db.init.lock` | 0B (mtime 2026-07-02 20:36) | none | **REMOVED** (stale empty, unheld; reappeared after t_6859d4bc) |
| `~/.hermes/gateway.lock` | 163B (`pid:5634`, kind hermes-gateway, `--replace`) | held by `python3.1` PID 5634 | **KEPT** (live process) |
| `~/.hermes/kanban/.dispatcher.lock` | 0B | held by gateway PID 5634 | **KEPT** (live dispatcher/gateway) |
| `~/.hermes/cron/.tick.lock` | 0B (mtime today tick) | none (tick file, live cron cycle) | **KEPT** (runtime tick placeholder) |
| `~/.hermes/cron/.jobs.lock` | 0B | none | **KEPT** (cron jobs placeholder; not empty-stale root lock pattern) |
| `~/.hermes/memories/*.lock`, `skills/.usage.json.lock` | 0B ancient | none | **LEFT** (not part of prior lock hygiene set; no rm -rf / broad delete) |
| `.grkr/locks/` | empty dir | n/a | nothing to remove |
| repo root `*.lock` | none | n/a | nothing |
| `build/*.lock` / `build/packages/gleam.lock` | present | n/a | **LEFT** (normal Gleam package locks) |

### Worktrees
`git worktree list` — only main `/Users/claw/work/grkr-v2-cron` on `v2` @ `bd2bc96`.
`git worktree prune --dry-run` — noop. `.worktrees/` empty (expected).

### Docs tip sync
No functional/docs HEAD drift vs tip: gleam-migration/README already synced to code baseline **454a65a** via t_4e3d6c6a (commit `bd2bc96`). No further docs tip rewrite this run — only this audit note.

### Documented only (not deleted — needs human approval if ever GC'd)
| Path | Notes |
|------|-------|
| `~/.hermes/kanban/workspaces/t_c55adf10` | prior note (May 26 grkr snapshot w/ build locks); still present from earlier audit |
| memory/skills zero-byte lock files | long-lived placeholders; out of prior hygiene set |

### Not done (out of scope)
No `rm -rf` of worktrees/kanban workspaces/large trees; no gateway/dispatcher/cron lock removal; no config.sh edits; no Linear work; no force-push; no PR #79 merge.

**Metadata:** `{locks_removed: ["~/.hermes/auth.lock","~/.hermes/kanban.db.dispatch.lock","~/.hermes/kanban.db.init.lock"], locks_kept: ["~/.hermes/gateway.lock","~/.hermes/kanban/.dispatcher.lock"], worktrees_pruned: 0, commit_sha: pending}`

# Generated by kanban task t_ee0b9cb6


## t_ee0b9cb6 retry (2026-07-03) — re-audit after timed_out prior run @ 816e0dd

**HEAD (pre):** `816e0dd` on `v2` (= `origin/v2`). Prior attempt on this card removed the same 3 hermes locks and committed the audit note, then timed out before `kanban_complete`.

### Locks audited (retry)
| Path | Size | lsof | Action |
|------|------|------|--------|
| `~/.hermes/auth.lock` | 0B (mtime 2026-07-03 03:01) | none | **REMOVED** (reappeared empty/unheld after 816e0dd) |
| `~/.hermes/kanban.db.dispatch.lock` | 0B (mtime 2026-07-03 02:42) | none | **REMOVED** (reappeared empty/unheld) |
| `~/.hermes/kanban.db.init.lock` | 0B (mtime 2026-07-03 03:01) | none | **REMOVED** (reappeared empty/unheld) |
| `.grkr/locks/main.lock` | 0B (mtime 2026-07-03 02:41) | none | **REMOVED** (new empty project lock; unheld) |
| `~/.hermes/gateway.lock` | 163B | held by `python3.1` PID 5634 | **KEPT** (live hermes-gateway) |
| `~/.hermes/kanban/.dispatcher.lock` | 0B | held by gateway PID 5634 | **KEPT** (live) |
| `~/.hermes/cron/.tick.lock`, `.jobs.lock` | 0B | none | **KEPT** (runtime placeholders) |
| memory/skills zero-byte locks | 0B ancient | none | **LEFT** (out of hygiene set) |
| `build/*.lock` / package locks | present | n/a | **LEFT** (normal Gleam package locks; 7 under build/) |
| `hermes-agent/flake.lock`, `uv.lock`, `venv/.lock` | package/tooling | none | **LEFT** (not runtime stale locks) |

### Worktrees
`git worktree list` — only main `/Users/claw/work/grkr-v2-cron` on `v2` @ `816e0dd`.
`git worktree prune --dry-run` — noop. `.worktrees/` empty (expected).

### Docs tip sync
No further docs HEAD rewrite; gleam-migration/README still at code baseline **454a65a** via t_4e3d6c6a. Only this re-audit note.

### Untracked (left alone)
`.grkr/kanban-cron-created-ids-2026-07-03.txt` — orchestrator id list, not hygiene.

### Not done (out of scope)
No `rm -rf` of worktrees/kanban workspaces/large trees; no gateway/dispatcher/cron lock removal; no Linear; no force-push; no PR #79 merge; no human-gate unblock.

**Metadata:** `{locks_removed: ["~/.hermes/auth.lock","~/.hermes/kanban.db.dispatch.lock","~/.hermes/kanban.db.init.lock",".grkr/locks/main.lock"], locks_kept: ["~/.hermes/gateway.lock","~/.hermes/kanban/.dispatcher.lock"], worktrees_pruned: 0, commit_sha: pending}`

# Generated by kanban task t_ee0b9cb6 (retry after timed_out run 14500)


## t_af712c27 (2026-07-03 08:42 PDT) — lock/worktree re-audit @ 2bc9a54 (PR #79)

**HEAD (pre):** `2bc9a54` on `v2` (= `origin/v2`). Prior hygiene cards (t_ee0b9cb6 and earlier) already pruned worktrees / noted locks; this is a re-audit only.

### Locks audited
| Path | Size | lsof | Action |
|------|------|------|--------|
| `~/.hermes/auth.lock` | 0B (mtime 2026-07-03 07:00) at audit | none | **GONE** by remove time (race/reaper; was empty/unheld when audited) |
| `~/.hermes/kanban.db.dispatch.lock` | 0B (mtime 2026-07-03 03:02) | none | **REMOVED** (reappeared empty/unheld) |
| `~/.hermes/kanban.db.init.lock` | 0B (mtime 2026-07-03 08:41) | none | **REMOVED** (reappeared empty/unheld) |
| `.grkr/locks/` | empty dir | n/a | **OK** (no reappeared project locks) |
| `~/.hermes/gateway.lock` | 163B | held by `python3.1` PID 5634 | **KEPT** (live hermes-gateway) |
| `~/.hermes/kanban/.dispatcher.lock` | 0B | held by gateway PID 5634 | **KEPT** (live) |
| `~/.hermes/cron/.tick.lock`, `.jobs.lock` | 0B | none | **KEPT** (runtime placeholders) |
| memory/skills/usage zero-byte locks | 0B | none | **LEFT** (out of hygiene set) |
| `build/*.lock` / package locks | present | n/a | **LEFT** (normal Gleam package locks) |
| `hermes-agent/flake.lock`, `uv.lock`, `venv/.lock` | package/tooling | none | **LEFT** (not runtime stale locks) |

### Worktrees
`git worktree list` — only main `/Users/claw/work/grkr-v2-cron` on `v2` @ `2bc9a54`.
`git worktree prune --dry-run` — noop. `.worktrees/` empty (expected).

### Docs tip sync
No further docs HEAD rewrite needed; tip already carries prior hygiene notes through `2bc9a54`. This note only.

### Untracked (left alone)
`.grkr/kanban-cron-created-ids-2026-07-03.txt`, `.grkr/kanban-cron-created-ids-2026-07-03-later.txt` — orchestrator id lists, not hygiene.

### Not done (out of scope)
No `rm -rf` of worktrees/kanban workspaces/large trees; no gateway/dispatcher/cron lock removal; no Linear; no force-push; no PR #79 merge; no human-gate unblock.

**Metadata:** `{locks_removed: ["~/.hermes/kanban.db.dispatch.lock","~/.hermes/kanban.db.init.lock"], locks_absent_at_rm: ["~/.hermes/auth.lock"], locks_kept: ["~/.hermes/gateway.lock","~/.hermes/kanban/.dispatcher.lock"], worktrees_pruned: 0, head_pre: "2bc9a54"}`

# Generated by kanban task t_af712c27


## t_e8848aeb (2026-07-03 14:45 PDT) — lock/worktree re-audit @ 6249b11 (PR #79)

**HEAD:** `6249b11` on `v2` (= `origin/v2`). Hygiene-only re-audit after prior cards (t_af712c27 and earlier). Orchestrator pre-check had already cleared reappeared locks; they reappeared again before this worker ran.

### Locks audited
| Path | Size | lsof | Action |
|------|------|------|--------|
| `~/.hermes/auth.lock` | 0B (mtime 2026-07-03 14:45) | none | **REMOVED** (reappeared empty/unheld) |
| `~/.hermes/kanban.db.dispatch.lock` | 0B (mtime 2026-07-03 14:45) | none | **REMOVED** (reappeared empty/unheld) |
| `~/.hermes/kanban.db.init.lock` | 0B (mtime 2026-07-03 14:44) | none | **REMOVED** (reappeared empty/unheld) |
| `.grkr/locks/` | empty dir | n/a | **OK** (no project locks) |
| `~/.hermes/gateway.lock` | 164B | held by `python3.1` PID 61453 | **KEPT** (live hermes-gateway) |
| `~/.hermes/kanban/.dispatcher.lock` | 0B | held by gateway PID 61453 | **KEPT** (live) |
| `~/.hermes/cron/.tick.lock`, `.jobs.lock` | 0B | none | **KEPT** (runtime placeholders) |
| memory/skills/usage zero-byte locks | 0B | none | **LEFT** (out of hygiene set) |
| `build/*.lock` / package locks | present | n/a | **LEFT** (normal Gleam package locks) |
| `hermes-agent/flake.lock`, `uv.lock`, `venv/.lock` | package/tooling | none | **LEFT** (not runtime stale locks) |
...
### Worktrees
`git worktree list` — only main `/Users/claw/work/grkr-v2-cron` on `v2` @ `6249b11`.
`git worktree prune --dry-run` — noop. `.worktrees/` empty (expected).

### Docs tip sync
Appended this note only (no further README/spec HEAD rewrite).

### Untracked (left alone)
`.grkr/kanban-cron-created-ids-2026-07-03.txt`, `-later.txt`, `-evening.txt` — orchestrator id lists, not hygiene.

### Not done (out of scope)
No broad `rm -rf`; no gateway/dispatcher/cron lock removal; no Linear; no force-push; no PR #79 merge; no human-gate unblock.

**Metadata:** `{locks_removed: ["~/.hermes/auth.lock","~/.hermes/kanban.db.dispatch.lock","~/.hermes/kanban.db.init.lock"], locks_kept: ["~/.hermes/gateway.lock","~/.hermes/kanban/.dispatcher.lock"], worktrees_pruned: 0, head_pre: "6249b11"}`

# Generated by kanban task t_e8848aeb


## t_95227cf0 (2026-07-04 02:48 PDT) — lock/worktree re-audit @ 7307c9b (PR #79)

**HEAD:** `7307c9b` on `v2` (= `origin/v2`). Hygiene-only re-audit after prior cards (t_e8848aeb and earlier). Orchestrator pre-check had cleared the usual empty runtime locks; they reappeared before this worker ran.

### Locks audited
| Path | Size | lsof | Action |
|------|------|------|--------|
| `~/.hermes/auth.lock` | 0B | none | **REMOVED** (reappeared empty/unheld) |
| `~/.hermes/kanban.db.dispatch.lock` | 0B | none | **REMOVED** (reappeared empty/unheld) |
| `~/.hermes/kanban.db.init.lock` | 0B | none | **REMOVED** (reappeared empty/unheld) |
| `.grkr/locks/` | empty/no `*.lock` | n/a | **OK** (no project locks) |
| `~/.hermes/gateway.lock` | 164B | held by `python3.1` PID 61453 | **KEPT** (live hermes-gateway) |
| `~/.hermes/kanban/.dispatcher.lock` | 0B | held by gateway PID 61453 | **KEPT** (live dispatcher) |
| `~/.hermes/cron/.tick.lock`, `.jobs.lock` | 0B | none | **KEPT** (runtime placeholders) |
| memory/skills/usage zero-byte locks | 0B | none | **LEFT** (out of hygiene set) |
| `build/*.lock` / package locks | present | n/a | **LEFT** (normal Gleam/package locks) |
| `hermes-agent/flake.lock`, `uv.lock`, package `yarn.lock` | package/tooling | none | **LEFT** (not runtime stale locks) |

### Worktrees
`git worktree list --porcelain` — only main `/Users/claw/work/grkr-v2-cron` on `v2` @ `7307c9b`.
`git worktree prune --dry-run` — noop. `.worktrees/` empty (expected).

### Docs tip sync
Appended this note only (no README/spec change warranted for non-functional hygiene audit).

### Untracked (left alone)
`.grkr/kanban-cron-created-ids-2026-07-03.txt`, `-later.txt`, `-evening.txt`, `-2026-07-04.txt` — orchestrator id lists, not hygiene.

### Not done (out of scope)
No broad `rm -rf`; no gateway/dispatcher/cron lock removal; no Linear; no force-push; no PR #79 merge; no human-gate unblock.

**Metadata:** `{locks_removed: ["~/.hermes/auth.lock","~/.hermes/kanban.db.dispatch.lock","~/.hermes/kanban.db.init.lock"], locks_kept: ["~/.hermes/gateway.lock","~/.hermes/kanban/.dispatcher.lock","~/.hermes/cron/.tick.lock","~/.hermes/cron/.jobs.lock"], worktrees_pruned: 0, head_pre: "7307c9b"}`

# Generated by kanban task t_95227cf0


## t_f5732197 (2026-07-04 08:52 PDT) — lock/worktree re-audit @ e2f08ec (PR #79)

**HEAD:** `e2f08ec` on `v2` (= `origin/v2`). Hygiene-only re-audit after prior cards; no feature work.
...
| `.grkr/locks/` | empty/no `*.lock` | n/a | **OK** (no project locks) |
| `~/.hermes/gateway.lock` | 164B | held by `python3.1` PID 61453 | **KEPT** (live hermes-gateway) |
| `~/.hermes/kanban/.dispatcher.lock` | 0B | held by `python3.1` PID 61453 | **KEPT** (live dispatcher) |
| `~/.hermes/cron/.tick.lock`, `.jobs.lock` | 0B | none | **KEPT** (runtime cron locks/placeholders) |
| package/tool locks (`hermes-agent/uv.lock`, `flake.lock`, `yarn.lock`, build locks) | package/tooling | n/a | **LEFT** (not stale runtime locks) |

### Worktrees
`git worktree list --porcelain` — only main `/Users/claw/work/grkr-v2-cron` on `v2` @ `e2f08ec`.
`.worktrees/` exists and is empty (expected).

### Docs tip sync
Appended this note only (no README/spec change warranted for non-functional hygiene audit).

### Untracked (left alone)
`.grkr/kanban-cron-created-ids-2026-07-03.txt`, `-later.txt`, `-evening.txt`, `-2026-07-04.txt`, `-2026-07-04-midday.txt` — orchestrator id lists, not hygiene.

### Not done (out of scope)
No broad `rm -rf`; no gateway/dispatcher/cron lock removal; no Linear; no force-push; no PR #79 merge; no human-gate unblock.

**Metadata:** `{locks_removed: ["~/.hermes/auth.lock","~/.hermes/kanban.db.dispatch.lock","~/.hermes/kanban.db.init.lock"], locks_kept: ["~/.hermes/gateway.lock","~/.hermes/kanban/.dispatcher.lock","~/.hermes/cron/.tick.lock","~/.hermes/cron/.jobs.lock"], worktrees_pruned: 0, head_pre: "e2f08ec"}`

# Generated by kanban task t_f5732197


## t_d3726e86 (2026-07-04 14:54 PDT) — lock/worktree re-audit @ be76ee5 (PR #79)

**HEAD:** `be76ee5` on `v2` (= `origin/v2`). Hygiene-only re-audit after prior cards; no feature work.
...
### Worktrees
`git worktree list --porcelain` — only main `/Users/claw/work/grkr-v2-cron` on `v2` @ `be76ee5`.
`.worktrees/` exists and is empty (expected); `.git/worktrees` is absent.

### Docs tip sync
Appended this note only (no README/spec change warranted for non-functional hygiene audit).

### Untracked (left alone)
`.grkr/kanban-cron-created-ids-2026-07-03.txt`, `-later.txt`, `-evening.txt`, `-2026-07-04.txt`, `-2026-07-04-midday.txt`, `-2026-07-04-evening.txt` — orchestrator id lists, not hygiene.

### Not done (out of scope)
No broad `rm -rf`; no gateway/dispatcher/cron lock removal; no Linear; no force-push; no PR #79 merge; no human-gate unblock.

**Metadata:** `{locks_removed: ["~/.hermes/auth.lock","~/.hermes/kanban.db.dispatch.lock","~/.hermes/kanban.db.init.lock"], locks_kept: ["~/.hermes/gateway.lock","~/.hermes/kanban/.dispatcher.lock","~/.hermes/cron/.tick.lock","~/.hermes/cron/.jobs.lock"], worktrees_pruned: 0, head_pre: "be76ee5"}`

# Generated by kanban task t_d3726e86


## t_89935a31 (2026-07-04 20:04 PDT) — lock/worktree re-audit @ cbda523 (PR #79)

**HEAD:** `cbda523` on `v2` (= `origin/v2`). Hygiene-only re-audit after prior docs-only tip sync; no feature work.

### Locks audited
| Path | Size | lsof | Action |
|------|------|------|--------|
| `~/.hermes/kanban.db.dispatch.lock` | 0B | none | **REMOVED ONCE** (reappeared empty/unheld; later recreated as an empty runtime placeholder while kanban tooling continued) |
| `~/.hermes/auth.lock` | missing | n/a | **OK** |
| `~/.hermes/kanban.db.init.lock` | missing | n/a | **OK** |
| `.grkr/locks/` | empty/no `*.lock` | n/a | **OK** (no project locks) |
| `~/.hermes/gateway.lock` | 164B | held by `python3.1` PID 61453 | **KEPT** (live hermes-gateway) |
| `~/.hermes/kanban/.dispatcher.lock` | 0B | held by `python3.1` PID 61453 | **KEPT** (live dispatcher) |
| `~/.hermes/cron/.tick.lock`, `.jobs.lock` | 0B | not removed | **KEPT** (runtime cron locks/placeholders) |
| package/tool locks (`hermes-agent/uv.lock`, `flake.lock`, `yarn.lock`, build locks) | package/tooling | n/a | **LEFT** (not stale runtime locks) |

### Worktrees
`git worktree list --porcelain` — only main `/Users/claw/work/grkr-v2-cron` on `v2` @ `cbda523`.
`.worktrees/` exists and is empty (expected); no stale worktrees or prunable entries found.

### Docs tip sync
Appended this note only (no README/spec change warranted for non-functional hygiene audit).

### Untracked (left alone)
Existing `.grkr/kanban-cron-created-ids-*` files remain untracked and intentionally unstaged.

### Not done (out of scope)
No broad `rm -rf`; no gateway/dispatcher/cron lock removal; no Linear; no force-push; no PR #79 merge; no human-gate unblock.

**Metadata:** `{locks_removed_once: ["~/.hermes/kanban.db.dispatch.lock"], locks_reappeared_empty_unheld: ["~/.hermes/kanban.db.dispatch.lock"], locks_kept: ["~/.hermes/gateway.lock","~/.hermes/kanban/.dispatcher.lock","~/.hermes/cron/.tick.lock","~/.hermes/cron/.jobs.lock"], worktrees_pruned: 0, head_pre: "cbda523"}`

# Generated by kanban task t_89935a31

2026-07-05 t_c343c7c3: removed empty/unheld `~/.hermes/auth.lock`; left gateway.lock/gateway.pid alone; `git worktree prune` found only main worktree and empty `.worktrees/`.
2026-07-06 t_62b3354a: `.grkr/locks/{comments,issues,main,prs}.lock` were empty/unheld placeholders; `.worktrees/` empty and `git worktree list` only main worktree @ `fde6021`; removed stale empty/unheld `~/.hermes/auth.lock`, kept live `~/.hermes/gateway.lock`, left kanban dispatch runtime lock untouched, ran safe `git worktree prune` (no stale entries), skipped docs tip sync because README/docs tree already had unrelated dirty edits.

2026-07-06 t_f7d8d9c3: re-audited hygiene at tip `93122ef` (`v2...origin/v2`); `.grkr/locks/{comments,issues,main,prs}.lock` remain empty flock placeholders, `.worktrees/` is empty, and no stale repo worktrees/prunable entries were present. No destructive cleanup performed. Synced README.md + docs/gleam-migration.md top tip/HEAD one-liners from `4ae00bc` to `93122ef` while preserving the existing ~295-test snapshot scope.

2026-07-06 t_2cd42e85: re-audited hygiene at tip `85cdcc0` (`v2...origin/v2`); `.grkr/locks/{comments,issues,main,prs}.lock` remain empty unheld flock placeholders, `.worktrees/` is empty, and `git worktree list --porcelain` shows only the main `v2` checkout. Removed empty/unheld `~/.hermes/auth.lock`, `~/.hermes/kanban.db.dispatch.lock`, and `~/.hermes/kanban.db.init.lock`; kept live `~/.hermes/gateway.lock` and `~/.hermes/kanban/.dispatcher.lock`, and left cron runtime lock placeholders alone. Synced README.md + docs/gleam-migration.md top tip/HEAD one-liners from `93122ef` to `85cdcc0` while preserving the existing 295-test snapshot scope; added `.kanban-logs/` to the existing ephemeral scratch ignore list after npm-test diagnostics produced root scratch logs.

## t_8899beba hygiene re-audit (2026-07-08 morning)
...
- Left live gateway/dispatcher locks intact; `.grkr/locks/*` were 0-byte unheld placeholders.

## t_48e8e864 hygiene re-audit (2026-07-08 afternoon)
- HEAD `0ebee391693e3f1aa74934f3b9fc6a60633e7012` on `v2`; `git worktree list --porcelain` showed only the main checkout, `.worktrees/` was empty, and `git worktree prune --verbose` had nothing to prune.
- `.grkr/locks/{comments,issues,main,prs}.lock` were 0-byte unheld placeholders. Removed empty, unheld Hermes lock placeholders after `lsof` showed no holders: /Users/claw/.hermes/auth.lock, /Users/claw/.hermes/kanban.db.dispatch.lock, /Users/claw/.hermes/kanban.db.init.lock.
- Kept live gateway/dispatcher locks (`~/.hermes/gateway.lock`, `~/.hermes/kanban/.dispatcher.lock`) held by gateway PID 61453. Synced README.md and docs/gleam-migration.md tip headers from `1332a8c` to `0ebee39`; left existing cron JSON/TXT scratch files untracked.

## t_df49c38c hygiene re-audit (2026-07-08 evening)
- HEAD `5e57f1abec27c19b49fe45e0ce8d7684b4bd137d` on `v2`; `git worktree list` showed only the main checkout, `.worktrees/` was empty, and `git worktree prune -v` had nothing to prune.
- `.grkr/locks/{comments,issues,main,prs}.lock` were 0-byte unheld placeholders (left as flock placeholders).
- Removed empty, unheld Hermes lock placeholders after `lsof` showed no holders: `/Users/claw/.hermes/auth.lock`, `/Users/claw/.hermes/kanban.db.dispatch.lock`, `/Users/claw/.hermes/kanban.db.init.lock`.
- Kept live gateway/dispatcher locks (`~/.hermes/gateway.lock`, `~/.hermes/kanban/.dispatcher.lock`) held by gateway PID 61453.
- Synced README.md and docs/gleam-migration.md tip headers from `0ebee39` to `5e57f1a`; left existing untracked `.grkr/kanban-cron-*` scratch JSON/TXT alone (not functional WIP).
- No Linear spawn, no PR merge, no destructive workspace rm.

## t_4f91bfcd hygiene re-audit (2026-07-09 morning)
- HEAD `e894354a553f10eecaafdfd51b191dea4c96fdb8` on `v2`; `git worktree list` showed only the main checkout, `.worktrees/` was empty, and `git worktree prune -v` had nothing to prune.
- `.grkr/locks/{comments,issues,main,prs}.lock` were 0-byte unheld placeholders (left as flock placeholders).
- Removed empty, unheld Hermes lock placeholders after `lsof` showed no holders: `/Users/claw/.hermes/auth.lock`, `/Users/claw/.hermes/kanban.db.dispatch.lock`, `/Users/claw/.hermes/kanban.db.init.lock`.
- Kept live gateway/dispatcher locks (`~/.hermes/gateway.lock`, `~/.hermes/kanban/.dispatcher.lock`) held by gateway PID 61453.
- Synced README.md and docs/gleam-migration.md tip headers from `5e57f1a` to `e894354` (includes flake-fix land after prior docs tip sync 97f49ad); left existing untracked `.grkr/kanban-cron-*` / `.grkr/e2e-logs/` scratch alone (not functional WIP).
- No Linear spawn, no PR merge, no destructive workspace rm.

## t_aa5cee90 hygiene re-audit (2026-07-09 afternoon)
- HEAD `fe3fcd1e137bd6b3d0c86dcb4b13893581c84f39` on `v2`; `git worktree list` showed only the main checkout, `.worktrees/` was empty, and `git worktree prune -v` had nothing to prune.
- `.grkr/locks/{comments,issues,main,prs}.lock` were 0-byte unheld placeholders (left as flock placeholders).
- Removed empty, unheld Hermes lock placeholders after `lsof` showed no holders: `/Users/claw/.hermes/auth.lock`, `/Users/claw/.hermes/kanban.db.dispatch.lock`, `/Users/claw/.hermes/kanban.db.init.lock` (init was briefly held by a short-lived python pid then free).
- Kept live gateway/dispatcher locks (`~/.hermes/gateway.lock`, `~/.hermes/kanban/.dispatcher.lock`) held by gateway PID 61453.
