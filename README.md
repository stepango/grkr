...
Current implementation status: see [docs/gleam-migration.md](./docs/gleam-migration.md) for v2 Gleam migration progress (github_picker/refusal/supervisor **GitHub** pick+schedule+spawn complete; Linear JobKey+spawn t_51747d23 @ **ce61881**; docs fixture e2e t_5254b489 / PR #92 @ **5782431**; `--linear-issue` research/plan MVP (t_48db8096 / PR #93) + multi-line KEY=val wire protocol on **main** tip **28e4794** (`
` escapes; **300** `gleam test` on MVP tip now on main; **298** pre-MVP @ ce61881 lineage); PR #79 **MERGED** 2026-07-10; Linear refuse progress dry-run planning (t_503ca0f3: `plan-linear-refusal` + Backlog state mutation plans + progress.json refuse parity) landed; implement/test/PR stages still follow-up; GitHub remains default `GRKR_ISSUE_PROVIDER`).
...
- Remaining work (Linear implement/test/PR after refuse progress planning t_503ca0f3; supervisor Linear pick→schedule→spawn already landed; GitHub default; spec/39 items 6–12 implemented; see gleam-migration.md table)
...
**High-level snapshot (@ main tip **`28e4794`** = PR #93 Linear research/plan MVP after PR #92 docs `5782431` + JobKey/spawn `ce61881` + PR #79 `78f4fb8`; pre-merge tip `62c73fb`; hygiene + worker-help + robot-main validation-failure + npm-order flake isolation; **304** gleam after Linear refuse progress planning t_503ca0f3 / **300** on MVP tip; GitHub path complete; Linear schedule/spawn + research/plan MVP + refuse dry-run mutations on tip; implement/test/PR still follow-up; see gleam-migration.md):**
