...
- Remaining work (Linear implement/test/PR after refuse dry-run land t_503ca0f3 + t_e47417cb / PR #95 @ **8aba009**; supervisor Linear pick→schedule→spawn already landed; GitHub default; spec/39 items 6–12 implemented; see gleam-migration.md table)
...
**High-level snapshot (@ main tip **`8aba009`** = PR #95 Linear refuse dry-run wire after PR #93 research/plan MVP `28e4794` + PR #92 docs `5782431` + JobKey/spawn `ce61881` + PR #79 `78f4fb8`; pre-merge tip `62c73fb`; hygiene + worker-help + robot-main validation-failure + npm-order flake isolation; **304** gleam on refuse tip / **300** on MVP tip; GitHub path complete; Linear schedule/spawn + research/plan MVP + refuse dry-run (t_503ca0f3 / t_e47417cb land t_18df994e) on tip; implement/test/PR still deferred; see gleam-migration.md):**
...
- `grkr --linear-issue <identifier>` (Linear MVP) loads Linear context without `gh issue view`, writes research/plan checkpoints, plans Linear comment mutations, and prepares an isolated worktree; implement/test/PR stages are deferred.
