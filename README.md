...
Current implementation status: see [docs/gleam-migration.md](./docs/gleam-migration.md) for v2 Gleam migration progress (github_picker/refusal/supervisor **GitHub** pick+schedule+spawn complete; Linear JobKey+spawn t_51747d23 @ **ce61881**; docs fixture e2e t_5254b489 / PR #92 @ **5782431**; `--linear-issue` research/plan MVP (t_48db8096 / PR #93 @ **28e4794** lineage) + multi-line KEY=val wire protocol; Linear refuse dry-run wire (t_503ca0f3 + t_e47417cb / PR #95 @ **8aba009** lineage); Linear implement dry-run (PR #97 @ **d1c1240** lineage) + test dry-run (PR #98) on **main** tip **bfee58c** (`
` escapes; **305** `gleam test` on test-stage tip; **304** on refuse tip / **300** on MVP tip / **298** pre-MVP @ ce61881 lineage); PR #79 **MERGED** 2026-07-10; Linear refuse + implement + test dry-run paths landed (`plan-linear-refusal` + `linear_flow` + provider-aware decision_gate/worker-refuse; implement STAGE + test STAGE dry-run mutations, worktree left); **publish + complete** still deferred; GitHub remains default `GRKR_ISSUE_PROVIDER`).
...
- Remaining work (Linear **publish + complete** after implement+test dry-run land PR #97/#98 @ **bfee58c**; refuse dry-run t_503ca0f3 + t_e47417cb / PR #95 @ **8aba009** lineage; supervisor Linear pick→schedule→spawn already landed; GitHub default; spec/39 items 6–12 implemented; see gleam-migration.md table)
...
**High-level snapshot (@ main tip **`bfee58c`** = PR #98 Linear test-stage dry-run after PR #97 implement `d1c1240` + PR #95 refuse `8aba009` + PR #93 MVP `28e4794`; GitHub path complete; Linear schedule/spawn + research/plan + refuse + implement + test (dry-run mutations, worktree left) landed; **305** gleam on tip; **publish + complete** still deferred; live `GRKR_LINEAR_MUTATE` still optional; see gleam-migration.md):**
...
- `grkr --linear-issue <identifier>` loads Linear context without `gh issue view`, writes research/plan checkpoints, plans Linear comment mutations, prepares an isolated worktree, runs the provider-aware decision gate, on proceed runs implement codex writing `implementation.log` + plans "In Progress" state mutation (dry-run), then continues to test stage: executes verification commands in worktree, writes `test.md` (Linear header), plans test comment + "In Review" state mutations (dry-run); publish/PR + complete still deferred.
