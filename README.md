...
- github_picker (client+main+picker + decoder_test 256/256 green post fixtures fix + hygiene M in client/decoder/field t_64f72de6 + t_077f26d0 (0 warnings fix for field/client/decoder.gleam)), refusal (flow/assessment/checkpoint + cli; assessment WIP t_639e3cb9), supervisor (main/loop/recovery/state/lock/config/phases 688 + exec.mjs t_639e3cb9 + pick.gleam 152LOC dispatching `GRKR_ISSUE_PROVIDER` github|linear + scheduler 130 + FFI) implemented + reviewed in slices; phases pick via `supervisor/pick` + sync/scan_pr/scan_comment/reap/cleanup
