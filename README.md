    ...
    20|- github_picker (client+main+picker), refusal (flow/assessment/checkpoint + cli), supervisor (main/loop/recovery/state/lock/config/phases 640LOC + scheduler 130 + FFI) implemented + reviewed in slices; phases.gleam fully expanded with sync/pick (real scheduler wired)/scan_pr/scan_comment (full)/reap/cleanup; workflow/ (task_log 237LOC + decision/worktree/main + FFIs) with sharding/persist/emit now wired into bin/grkr + grkr-issue-workflow.sh (t_ef6b855f)
    ...
    23|- Still thick: grkr-issue-workflow.sh (476 LOC thin wrapper; decision+task_log wired, worktree partial; thinning ongoing)
