...
Current implementation status: see [docs/gleam-migration.md](./docs/gleam-migration.md) for v2 Gleam migration progress (github_picker/refusal/supervisor **GitHub** pick+schedule+spawn complete; Linear JobKey+spawn t_51747d23 @ **ce61881**; docs fixture e2e t_5254b489 / PR #92 @ **5782431**; `--linear-issue` research/plan MVP (t_48db8096 / PR #93 @ **28e4794** lineage) + multi-line KEY=val wire protocol; Linear refuse dry-run wire (t_503ca0f3 + t_e47417cb / PR #95 @ **8aba009** lineage); Linear implement dry-run (PR #97 @ **d1c1240** lineage) + test dry-run (PR #98) + publish+complete dry-run (PR #100 @ **bd523a6**) + guarded live mutate (PR #104 @ **e28d0c0** lineage; nits PR #107 @ **8d4b674**; optional STRICT hard-fail PR #110 @ **82f3644**; GitHub process_issue thinning tip PR #117 @ **53592d4** — test checkpoint #112 @ **c438409** + publish helpers #115 @ **6e0f1d3** + research/plan ensure_checkpoint_stage extract + completion surface (t_d328b158 @ def63d8) + thin process_issue orchestrator (t_3619188b: `bin/grkr` 527→435, github_issue.sh; process_issue now clear launcher sequencing ensure_*/publish_*/completion helpers); docs tip-sync #120 @ **321aa11** after completion #119 @ **def63d8**; `GRKR_LINEAR_MUTATE=1` default OFF) on **main** lineage; GitHub PR from linear-* branch + Linear Done state + completion comment now wired after test success (dry-run default; live apply gated); GitHub remains default `GRKR_ISSUE_PROVIDER`).
