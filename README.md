...
` escapes; **304** `gleam test` on refuse tip; **300** on MVP tip / **298** pre-MVP @ ce61881 lineage); PR #79 **MERGED** 2026-07-10; Linear refuse dry-run path landed (`plan-linear-refusal` + `linear_flow` + provider-aware decision_gate/worker-refuse; progress.json refuse parity); implement/test/PR stages still deferred; GitHub remains default `GRKR_ISSUE_PROVIDER`).
...
**High-level snapshot (@ main tip **`8aba009`** lineage + Linear implement dry-run slice; PR #95 refuse + PR #93 MVP; GitHub path complete; Linear schedule/spawn + research/plan + refuse dry-run + implement decision/codex/In-Progress-plan (dry-run) landed; test stage + publish still deferred; see gleam-migration.md):**
...
- `grkr --linear-issue <identifier>` loads Linear context without `gh issue view`, writes research/plan checkpoints, plans Linear comment mutations, prepares an isolated worktree, runs the provider-aware decision gate, and on proceed runs implement codex writing `implementation.log` + plans "In Progress" state mutation (dry-run by default); test stage + publish/PR still deferred.
