...
Current implementation status: see [docs/gleam-migration.md](./docs/gleam-migration.md) for v2 Gleam migration progress (github_picker/refusal/supervisor **GitHub** pick+schedule+spawn complete; Linear JobKey+spawn t_51747d23 @ **ce61881**; docs fixture e2e t_5254b489 / PR #92 @ **5782431**; `--linear-issue` research/plan MVP (t_48db8096 / PR #93 @ **28e4794** lineage) + multi-line KEY=val wire protocol; Linear refuse dry-run wire (t_503ca0f3 + t_e47417cb / PR #95 @ **8aba009** lineage); Linear implement dry-run (PR #97 @ **d1c1240** lineage) + test dry-run (PR #98) + publish+complete dry-run (PR #100 @ **bd523a6**) + guarded live mutate (PR #104 @ **e28d0c0** lineage; nits PR #107 @ **8d4b674**; optional STRICT hard-fail PR #110 @ **82f3644**; GitHub process_issue thinning tip PR #115 @ **6e0f1d3** — test checkpoint PR #112 @ **c438409** + publish helpers extract → `bin/lib/github_issue.sh`, `bin/grkr` 875→738→639; `GRKR_LINEAR_MUTATE=1` default OFF) on **main** lineage; GitHub PR from linear-* branch + Linear Done state + completion comment now wired after test success (dry-run default; live apply gated); GitHub remains default `GRKR_ISSUE_PROVIDER`).
...
- Remaining work (Linear guarded live mutate **landed** @ **e28d0c0** / PR #104; nits **8d4b674** / PR #107; optional `GRKR_LINEAR_MUTATE_STRICT=1` **82f3644** / PR #110; GitHub process_issue thinning tip **6e0f1d3** / PR #115 — test checkpoint **c438409** / #112 + publish helpers extract, `bin/grkr` 738→639; `GRKR_LINEAR_MUTATE=1` enables apply, default OFF / dry-run; further `bin/grkr` thinning still open; refuse dry-run t_503ca0f3 + t_e47417cb / PR #95 @ **8aba009** lineage; supervisor Linear pick→schedule→spawn already landed; GitHub default; spec/39 items 6–12 implemented; see gleam-migration.md table)
...
- Bin updates/hygiene (per AGENTS: preserve sh conv, small explicit changes, <1000 LOC): **doctor.sh (51 LOC thin: doctor_init + delegate to `grkr/doctor/cli`)**, grkr-project-status.sh (81 LOC thin), grkr-issue-workflow.sh (80 LOC thin wrapper delegating workflow/* CLIs incl decision_gate + test_completion_marker), worker-handle-comment.sh (29 LOC thin), worker-pick-issue.sh (46 LOC), worker-sync-main.sh (18 LOC), worker-resolve-pr.sh (39 LOC), worker-refuse-issue.sh (40 LOC thin calling refusal/cli), robot-main.sh (57 LOC), grkr (now thinner post-extract), grkr-templates.sh (62 LOC thin), + bin/lib/{refusal_paths.sh,task_progress.sh,github_issue.sh} (github_issue.sh holds GitHub-only test checkpoint (slice 1) + publish helpers publish_issue_changes/extract_codex_pr_body/ensure_pr_body_limit (slice 3 of process_issue thinning per docs/design-github-process-issue-thinning.md); linear_issue.sh untouched; shared write_test_checkpoint_with_header + ensure_publishable_file_sizes stay in grkr for Linear reuse). grkr now ~639 LOC (thinner post publish extract).
