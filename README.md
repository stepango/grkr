...
Current implementation status: see [docs/gleam-migration.md](./docs/gleam-migration.md) for v2 Gleam migration progress (github_picker/refusal/supervisor **GitHub** pick+schedule+spawn complete; Linear JobKey+spawn t_51747d23 @ **ce61881**; docs fixture e2e t_5254b489 / PR #92 @ **5782431**; `--linear-issue` research/plan MVP (t_48db8096 / PR #93 @ **28e4794** lineage) + multi-line KEY=val wire protocol; Linear refuse dry-run wire (t_503ca0f3 + t_e47417cb / PR #95 @ **8aba009** lineage); Linear implement dry-run (PR #97 @ **d1c1240** lineage) + test dry-run (PR #98) + publish+complete dry-run (PR #100 @ **bd523a6**) + guarded live mutate (PR #104 @ **e28d0c0** lineage; nits PR #107 @ **8d4b674**; optional STRICT hard-fail PR #110 @ **82f3644**; GitHub process_issue product tip PR #121 @ **a3d9702** — test checkpoint #112 @ **c438409** + publish helpers #115 @ **6e0f1d3** + research/plan #117 @ **53592d4** + completion surface #119 @ **def63d8** + thin process_issue orchestrator (t_3619188b / PR #121: `bin/grkr` 527→435, github_issue.sh; process_issue now clear launcher sequencing ensure_*/publish_*/completion helpers); docs tip-sync #122 @ **bdf396b** after product #121 @ **a3d9702**; Linear thinning design tip PR #123 @ **ad3e9a4** + docs tip-sync #124 @ **729abd0**; Linear first extract product tip PR #125 @ **7721b61** (t_9c0c6ab9: `ensure_linear_test_checkpoint` → `linear_issue_stages.sh`, 923→770) + docs tip-sync #126 @ **57eef93**; Linear second extract product tip PR #127 @ **8ae5250** (t_8ca53b63: `ensure_linear_publish_complete` → `linear_issue_stages.sh`, 770→613 / stages 357) + docs tip-sync #128 @ **1ac9be2**; Linear third extract product tip PR #129 @ **ce34b29** (t_7d3260b2: `ensure_linear_refusal_checkpoint` → `linear_issue_stages.sh`, 613→495 / stages 357→483) + docs tip-sync #130 @ **b97fecc**; Linear fourth extract product tip PR #131 @ **48aa21b** (t_81b53e16: `ensure_linear_checkpoint_stage` + `ensure_linear_implement_in_progress` → `linear_issue_stages.sh`, 495→386 / stages 483→599) + docs tip-sync #132 @ **3e7cbc5**; Linear fifth/final thin sequencer product tip PR #133 @ **f6b34d4** (t_d9eb82bb: thin `process_linear_issue` sequencer, `linear_issue.sh` 386→329 / stages 599→725) + docs tip-sync #134 @ **5418159**; shared helpers first extract PR #136 @ **d04f5e9** (test-write) + docs tip-sync #137 @ **5e53aaf**; shared helpers second extract product tip PR #138 @ **593e18b** (t_9e60aed0: line-limit + ensure_publishable → `bin/lib/issue_shared.sh`, `bin/grkr` 339→279 / issue_shared 110→178) + docs tip-sync #139 @ **ce37e6a**; shared helpers third extract product tip PR #140 @ **325ee9a** (t_2091085e: run_codex_prompt → `bin/lib/issue_shared.sh`, `bin/grkr` 279→260 / issue_shared 178→198) + docs tip-sync #141 @ **36de1a1**; shared helpers fourth extract product tip PR #142 @ **f6fb872** (t_bc6ebfda: run_progress_cli + checkpoint_marker → `bin/lib/issue_shared.sh`, `bin/grkr` 260→233 / issue_shared 198→228) + docs tip-sync #143 @ **3b0b2df**; shared helpers fifth extract product tip PR #144 @ **c801967** (t_2847ff4d: attach_issue_logs → `bin/lib/issue_shared.sh` (verbatim; no wrapper), `bin/grkr` 233→198 / issue_shared 228→249; summarize_text dead hygiene removed) + docs tip-sync #145 @ **9a1b8f6**; github_issue PR body helpers design tip PR #146 @ **15a5050**; github_issue PR body helpers product tip PR #147 @ **1216e94** (t_e06571e3: ensure_pr_body_limit + extract_codex_pr_body → Gleam progress/templates + cli + thin delegates; `github_issue.sh` 545→543 / templates 176→221) + docs tip-sync #148 @ **745ca83**; docs tip-sync #151 @ **c918cde** after coding-agent #149/#150; docs tip-sync #153 @ **4613b02** after github_issue completion polish **29c7a4b** / PR #152; supervisor phases LOC hygiene product tip PR #154 @ **0d13a98** (t_94976f9c: phases.gleam 688→117 thin dispatcher + 7 concern modules; zero behavior change) + docs tip-sync #155 @ **d02734b**; progress/main LOC hygiene product tip PR #156 @ **71075c2** (t_8c7cd0a0: main.gleam 644→63 thin facade + checkpoint_plan/linear_plan/templates_cli/linear_apply; zero behavior change) + docs tip-sync #157 @ **a426932**; handle_comment LOC hygiene product tip PR #158 @ **e358311** (t_a12fcdec: handle_comment.gleam 456→92 thin facade + 6 concern modules; zero behavior change) + docs tip-sync #159 @ **3216246**; supervisor/comment_handler LOC hygiene product tip PR #160 @ **178c535** (t_a9bf1687: comment_handler.gleam 430→91 thin facade + 6 concern modules; zero behavior change, stable grkr/supervisor/comment_handler entry) + docs tip-sync #162 @ **9b36632**; resolve_pr/main LOC hygiene product tip PR #164 @ **cfdfb76** + docs tip-sync #165 @ **ce5830c**; deploy Docker+Helm PR #166 @ **deb0acc**; linear_issue_stages stages-split design tip PR #167 @ **6250788**; linear_issue_stages stages-split slice 1 product tip PR #168 @ **34ab0f8** (t_63144916) + docs tip-sync #169 @ **55ee02b**; linear_issue_stages stages-split slice 2 product tip PR #170 @ **acd2910** (t_5ed4be3d: `ensure_linear_test_checkpoint` → `linear_issue_stages_test.sh`; facade sources refusal+test; stages ~623→~472 + test ~176; remaining bodies until slices 3–5; lineage resolve_pr **cfdfb76** / #164); slice 3 publish extract product tip **feed4ea** / PR #173 (t_e0c51a35: `ensure_linear_publish_complete` → `linear_issue_stages_publish.sh`; facade sources refusal+test+publish; stages ~472→~323 + publish ~175); linear_issue_stages stages-split slice 4 product tip PR #175 @ **249dbbd** (t_952ad862: `ensure_linear_checkpoint_stage` + `ensure_linear_implement_in_progress` → `linear_issue_stages_research_plan.sh`; facade sources refusal+research_plan+test+publish; stages ~323→~218 + research_plan ~125; lineage tip-sync #174 @ **6eb24bf** after slice 3 **feed4ea** / #173 + docs tip-sync #176 @ **f25df23**); linear_issue_stages stages-split slice 5 product tip PR #177 @ **cb6b1b5** (t_ffc321af: `run_linear_decision_stage` + `handle_linear_decision_refuse` + `run_linear_implement_stage` → `linear_issue_stages_implement.sh`; facade source-only refusal→research_plan→implement→test→publish; stages ~225→~88 + implement ~133; stages-split complete); GitHub stages-split slice 1 product tip PR #181 @ **6ef9f1d** (t_f8ceeffc: research/plan → `github_issue_stages_research_plan.sh`; facade sources research_plan; `github_issue.sh` 542→459 + research_plan 111; design **d51ae73** / #180); GitHub stages-split slice 2 product tip PR #183 @ **b19a5ba** (t_be3ee540: test checkpoint → `github_issue_stages_test.sh`; facade sources research_plan then test; `github_issue.sh` 459→332 + test 154); GitHub stages-split slice 3 product tip PR #186 @ **b4ecc04** (t_f2a425d1: publish+completion → `github_issue_stages_publish.sh`; facade sources research_plan then test then publish fail-closed; `github_issue.sh` 332→210 + publish 151); GitHub stages-split slice 4 product tip PR #189 @ **6dc13ac** (t_3fdd43d1: bootstrap/decision/refuse/implement/finalize → `github_issue_stages_implement.sh`; facade source-only research_plan→implement→test→publish; `github_issue.sh` 210→71 + implement 173; research_plan 111 + test 154 + publish 152 unchanged; stages-split **complete**); docs tip-sync #190 @ **c0d4d5d** after #189; issue_shared concern-split design tip PR #191 @ **a594167**; docs tip-sync #193 @ **d706ebd** after #192; issue_shared concern-split design tip PR #191 @ **a594167**; issue_shared concern-split slice 1 product tip PR #195 @ **1dca874** + docs tip-sync #196 @ **bb07d51**; issue_shared concern-split slice 2 product tip PR #197 @ **7b4ac5e** (t_28a87184: progress → `issue_shared_progress.sh`; facade sources attach then progress fail-closed; design **a594167** / #191) + docs tip-sync #198 @ **6f199d6**; issue_shared concern-split slice 3 product tip **f9ef4ed** / PR #199 (t_8ecf03f2: line-limit → `issue_shared_line_limit.sh`; facade sources attach then progress then line_limit fail-closed) + docs tip-sync #200 @ **3d06938**; issue_shared concern-split slice 4 product tip **4b71cf1** / PR #201 (test-write → `issue_shared_test_write.sh`) + docs tip-sync #202 @ **b2d140d**; doctor/validate concern-split slice 2 product tip **88e3eea** / PR #220 (t_74a7a161: 387→67 thin facade + tools/agent/config/run; design **cbf74fc** / #217) + parent coding_agent concern-split slice 1 product tip **20310e2** / PR #218 (t_e7ea2b4b: 399→105 thin facade + types/select/classify/run; design **cbf74fc** / #217) + parent coding-agent polish P0+P1 product tip **b49a072** / PR #215 (t_49452ed3: doctor COMMENT/RESOLVE template + GROK_MODEL default grok-4.5; design **910ba27** / #214) + parent Gleam coding-agent swap slice 2 product tip **c55f8e1** / PR #212 (t_beed04ab: `resolve_pr` honors `GRKR_CODING_AGENT` via `coding_agent` Resolve/ConflictResolve; design **a87f379** / #207) + parent slice 1 **4553356** / PR #210 (t_45efd0d1 comment classify) + parent issue_shared concern-split slice 5 FINAL **851bed2** / PR #203 (t_3d89a6b9: coding-agent → `issue_shared_coding_agent.sh`; facade source-only coding_agent→progress→test_write→line_limit→attach fail-closed; concern-split **complete**; design **a594167** / #191) + docs tip **caef425** / PR #206 (checkpoint-json Gleam helpers **NO-GO**) + lineage docs #205 @ **b3c614f** (coding-agent matrix eval refresh) + tip-sync #204 @ **48f25e9**; progress/linear_mutation LOC hygiene product tip PR #192 @ **7053645** (t_d0a2d481: 440→44 facade + types 29 + build 117 + parse 183 + policy 148; zero behavior change); swappable coding agent product tip PR #149 @ **d55dd73** (t_*: Codex↔Grok via `GRKR_CODING_AGENT` + run_coding_agent_prompt bridge); coding-agent matrix + quality eval harness product tip PR #150 @ **1edf636** (per-step `GRKR_AGENT_{DECISION,IMPLEMENT,REMEDIATE}` + `scripts/coding-agent-eval-matrix.sh`); `GRKR_LINEAR_MUTATE=1` default OFF; GitHub PR from linear-* + Linear Done+comment (dry-run default); GitHub default `GRKR_ISSUE_PROVIDER`).; fifth/final thin process_linear_issue sequencer (386→329 / 599→725; t_d9eb82bb) complete like GitHub a3d9702)
## Gleam v2 Migration Progress

**GitHub-only first; PR #79 (https://github.com/stepango/grkr/pull/79) MERGED to `main` @ `78f4fb8` (2026-07-10T23:46:23Z).** GitHub picker/refusal/supervisor path lives on **main**. Follows AGENTS.md: thick Gleam in `src/grkr/`, thin shell wrappers in `bin/` (preserve conventions), files <=1000 LOC, `spec/parts/` canonical. Large tasks decomposed into kanban slices to handle iteration budgets (90/90 max_iterations exhaustion on complex parents).
...
- Remaining work (issue_shared concern-split slice 1 **landed** @ **1dca874** / PR #195 (t_c405d9ff): attach → `issue_shared_attach.sh` + docs tip-sync #196 @ **bb07d51**; slice 2 **landed** @ **7b4ac5e** / PR #197 (t_28a87184): progress → `issue_shared_progress.sh` + docs tip-sync #198 @ **6f199d6**; slice 3 **landed** @ **f9ef4ed** / PR #199 (t_8ecf03f2): line-limit → `issue_shared_line_limit.sh` + docs tip-sync #200 @ **3d06938**; slice 4 **landed** @ **4b71cf1** / PR #201 (t_0a05c8e5): test-write → `issue_shared_test_write.sh` + docs tip-sync #202 @ **b2d140d**; slice 5 FINAL **landed** @ **851bed2** / PR #203 (t_3d89a6b9): coding-agent → `issue_shared_coding_agent.sh` (facade source-only coding_agent→progress→test_write→line_limit→attach fail-closed; concern-split **complete**; design **a594167** / #191) + Gleam coding-agent swap slice 1 **landed** @ **4553356** / PR #210 (t_45efd0d1 comment classify; design **a87f379** / #207) + coding_agent concern-split slice 1 **landed** @ **20310e2** / PR #218 (t_e7ea2b4b; design **cbf74fc** / #217) + doctor/validate concern-split slice 2 **landed** @ **88e3eea** / PR #220 (t_74a7a161) + coding-agent polish P0+P1 **landed** @ **b49a072** / PR #215 (t_49452ed3; design **910ba27** / #214) + Gleam coding-agent swap slice 2 **landed** @ **c55f8e1** / PR #212 (t_beed04ab: `resolve_pr` honors `GRKR_AGENT_RESOLVE` / `GRKR_CODING_AGENT` via `coding_agent` Resolve/ConflictResolve; Codex default argv parity; design **a87f379** / #207) + docs tip-sync #208 @ **71e329d** after docs tip **caef425** / PR #206 (checkpoint-json Gleam helpers **NO-GO**) + lineage docs #205 @ **b3c614f** (coding-agent matrix eval refresh) + tip-sync #204 @ **48f25e9**; remaining pure Gleam: checkpoint-json **NO-GO** (leave shell jq; see [`docs/design-checkpoint-json-gleam.md`](docs/design-checkpoint-json-gleam.md) / #206 @ **caef425**); docs tip-sync #193 @ **d706ebd** after #192; progress/linear_mutation LOC hygiene **landed** @ **7053645** / PR #192 (t_d0a2d481): 440→44 facade + types 29 + build 117 + parse 183 + policy 148; remaining pure Gleam: checkpoint-json **NO-GO** (leave shell jq; see [`docs/design-checkpoint-json-gleam.md`](docs/design-checkpoint-json-gleam.md) / #206 @ **caef425**); Linear guarded live mutate **landed** @ **e28d0c0** / PR #104; nits **8d4b674** / PR #107; optional `GRKR_LINEAR_MUTATE_STRICT=1` **82f3644** / PR #110; GitHub process_issue thinning complete (product tip **a3d9702** / PR #121 after **def63d8** / #119: thin orchestrator; `bin/grkr` 527→435); Linear thinning design tip **ad3e9a4** / PR #123 + docs tip-sync #124 @ **729abd0**; Linear first extract product tip **7721b61** / PR #125 + docs tip-sync #126 @ **57eef93**; Linear second extract product tip **8ae5250** / PR #127 (t_8ca53b63: `ensure_linear_publish_complete` → `bin/lib/linear_issue_stages.sh`, `linear_issue.sh` 770→613 / stages 357) + docs tip-sync #128 @ **1ac9be2**; Linear third extract product tip **ce34b29** / PR #129 (t_7d3260b2: `ensure_linear_refusal_checkpoint` → `bin/lib/linear_issue_stages.sh`, `linear_issue.sh` 613→495 / stages 357→483) + docs tip-sync #130 @ **b97fecc**; Linear fourth extract product tip **48aa21b** / PR #131 (t_81b53e16: `ensure_linear_checkpoint_stage` + `ensure_linear_implement_in_progress` → `bin/lib/linear_issue_stages.sh`, `linear_issue.sh` 495→386 / stages 483→599) + docs tip-sync #132 @ **3e7cbc5**; Linear fifth/final thin sequencer product tip **f6b34d4** / PR #133 (t_d9eb82bb: thin `process_linear_issue` sequencer, `linear_issue.sh` 386→329 / stages 599→725) + docs tip-sync #134 @ **5418159**; shared helpers first extract product tip **d04f5e9** / PR #136 (t_d9c18700: test-write cluster → `bin/lib/issue_shared.sh`, `bin/grkr` 435→339 / issue_shared 110; design **d90fbaf** / PR #135) + docs tip-sync #137 @ **5e53aaf**; shared helpers second extract product tip **593e18b** / PR #138 (t_9e60aed0: line-limit + ensure_publishable → `bin/lib/issue_shared.sh`, `bin/grkr` 339→279 / issue_shared 110→178) + docs tip-sync #139 @ **ce37e6a**; shared helpers third extract product tip **325ee9a** / PR #140 (t_2091085e: run_codex_prompt → `bin/lib/issue_shared.sh`, `bin/grkr` 279→260 / issue_shared 178→198) + docs tip-sync #141 @ **36de1a1**; shared helpers fourth extract product tip **f6fb872** / PR #142 (t_bc6ebfda: run_progress_cli + checkpoint_marker → `bin/lib/issue_shared.sh`, `bin/grkr` 260→233 / issue_shared 198→228) + docs tip-sync #143 @ **3b0b2df**; shared helpers fifth extract product tip **c801967** / PR #144 (t_2847ff4d: attach_issue_logs → `bin/lib/issue_shared.sh`, `bin/grkr` 233→198 / issue_shared 228→249) + docs tip-sync #145 @ **9a1b8f6**; github_issue thinning design tip **15a5050** / PR #146; github_issue PR body helpers product tip **1216e94** / PR #147 (t_e06571e3: ensure_pr_body_limit + extract_codex_pr_body → Gleam progress/templates + cli + thin delegates; `github_issue.sh` 545→543 / templates 176→221) + docs tip-sync #148 @ **745ca83**; swappable coding agent product tip **d55dd73** / PR #149; coding-agent matrix + quality eval harness product tip **1edf636** / PR #150 (per-step agent matrix + `scripts/coding-agent-eval-matrix.sh`) + docs tip-sync #151 @ **c918cde**; github_issue completion summary polish product tip **29c7a4b** / PR #152 (t_dec62328: render_github_completion_summary → Gleam + thin post_completion_comment; `github_issue.sh` 543→542 / templates 221→238) + docs tip-sync #153 @ **4613b02**; supervisor phases LOC hygiene product tip **0d13a98** / PR #154 (t_94976f9c: phases.gleam 688→117 + 7 concern modules) + docs tip-sync #155 @ **d02734b**; progress/main LOC hygiene product tip **71075c2** / PR #156 (t_8c7cd0a0: main.gleam 644→63 thin facade + 4 concern modules) + docs tip-sync #157 @ **a426932**; handle_comment LOC hygiene product tip **e358311** / PR #158 (t_a12fcdec: handle_comment.gleam 456→92 thin facade + 6 concern modules) + docs tip-sync #159 @ **3216246**; supervisor/comment_handler LOC hygiene product tip **178c535** / PR #160 (t_a9bf1687: comment_handler.gleam 430→91 thin facade + 6 concern modules) + docs tip-sync #162 @ **9b36632**; resolve_pr/main LOC hygiene product tip **cfdfb76** / PR #164 + docs tip-sync #165 @ **ce5830c**; deploy Docker+Helm **deb0acc** / PR #166; linear_issue_stages stages-split design tip **6250788** / PR #167; linear_issue_stages stages-split slice 1 product tip **34ab0f8** / PR #168 (t_63144916) + docs tip-sync #169 @ **55ee02b**; linear_issue_stages stages-split slice 2 product tip **acd2910** / PR #170 (t_5ed4be3d: `ensure_linear_test_checkpoint` → `bin/lib/linear_issue_stages_test.sh`; facade sources refusal+test; stages ~623→~472 + test ~176); slice 3 publish extract product tip **feed4ea** / PR #173 (t_e0c51a35: `ensure_linear_publish_complete` → `bin/lib/linear_issue_stages_publish.sh`; facade sources refusal+test+publish; stages ~472→~323 + publish ~175); linear_issue_stages stages-split slice 4 product tip **249dbbd** / PR #175 (t_952ad862) + docs tip-sync #176 @ **f25df23**; linear_issue_stages stages-split slice 5 product tip **cb6b1b5** / PR #177 (t_ffc321af: `run_linear_decision_stage` + `handle_linear_decision_refuse` + `run_linear_implement_stage` → `bin/lib/linear_issue_stages_implement.sh`; facade source-only; stages ~225→~88 + implement ~133; stages-split complete); lineage tip-sync #174 @ **6eb24bf** after slice 3 **feed4ea** / #173); GitHub vertical: pure Gleam checkpoint-json **NO-GO** (leave shell jq; see [`docs/design-checkpoint-json-gleam.md`](docs/design-checkpoint-json-gleam.md) / #206 @ **caef425**); **next shell LOC hygiene** GitHub stages-split slice 1 research_plan **landed** @ **6ef9f1d** / PR #181; slice 2 test checkpoint **landed** @ **b19a5ba** / PR #183 (t_be3ee540); slice 3 publish+completion extract **landed** @ **b4ecc04** / PR #186 + docs tip-sync #187 @ **3c489c7** (t_f2a425d1: `publish_issue_changes` + alias + `ensure_pr_body_limit` + `extract_codex_pr_body` + `post_completion_comment` + alias → `github_issue_stages_publish.sh`; facade sources research_plan then test then publish; `github_issue.sh` 332→210 + publish 151); slice 4 implement/bootstrap/decision/finalize extract **landed** @ **6dc13ac** / PR #189 (t_3fdd43d1: `bootstrap_github_issue_task` + `run_github_decision_stage` + `handle_github_decision_refuse` + `run_github_implement_stage` + `finalize_github_issue_complete` → `github_issue_stages_implement.sh`; facade source-only research_plan→implement→test→publish; `github_issue.sh` 210→71 + implement 173; research_plan 111 + test 154 + publish 152 unchanged; stages-split **complete**; issue_shared concern-split slice 1 **landed** @ **1dca874** / PR #195 (t_c405d9ff: attach → `issue_shared_attach.sh`; facade sources attach; design **a594167** / #191); design [`docs/design-github-issue-stages-split.md`](docs/design-github-issue-stages-split.md) **d51ae73** / #180); `GRKR_LINEAR_MUTATE=1` enables apply, default OFF / dry-run; refuse dry-run t_503ca0f3 + t_e47417cb / PR #95 @ **8aba009** lineage; supervisor Linear pick→schedule→spawn already landed; GitHub default; spec/39 items 6–12 implemented; see gleam-migration.md table)- Traceability to specific kanban tasks (e.g. t_58ea0e02 scheduler impl, t_20695489 test+docs+sync, t_78a7818e cleanup prune, t_767a0b08 prior test+docs, t_65d650b7 review (supervisor slice), t_55147911 docs follow-up, t_e56d835b hygiene commit+push v2 uncommitted, t_2c94e927 (decision_gate + handle_comment wire/validate per spec/22+15 + t_1cca18ff), t_0843d707 (test_stage.gleam + wiring per spec/39 item 9 / #18 + spec/17), t_e2282d3f (gleam build 0 warnings fix in handle_comment WIP), PR#79 reviews t_2abfcacc/t_e1b63fc6/t_1ef6c1a8 etc)
- Design refs (supervisor-design-final.md, supervisor-synthesis.md, gleam-migration-patterns.md)
- Lock audit notes from this run

**High-level snapshot (Linear publish+complete dry-run landed after test; GitHub path complete; Linear schedule/spawn + research/plan + refuse + implement + test + publish/complete (dry-run default: PR from linear-* + Done state plan + completion comment) landed; guarded live `GRKR_LINEAR_MUTATE=1` apply **landed** @ **e28d0c0** / PR #104 + nits **8d4b674** / PR #107 + optional STRICT **82f3644** / PR #110 (default OFF; STRICT=1 hard-fails non-refuse apply failures, refuse.* stays soft); GitHub process_issue product tip **a3d9702** / PR #121 (`bin/lib/github_issue.sh` holds test+publish+checkpoints+completion + decision/implement stages; process_issue thin orchestrator in bin/grkr 527→435; lineage **def63d8** / #119, docs #122 @ **bdf396b**); docs tip **729abd0** / PR #124 after design **ad3e9a4** / PR #123; Linear first extract product tip **7721b61** / PR #125 + docs tip-sync #126 @ **57eef93**; Linear second extract product tip **8ae5250** / PR #127 (t_8ca53b63 `ensure_linear_publish_complete` → `linear_issue_stages.sh`, 770→613 / stages 357) + docs tip-sync #128 @ **1ac9be2**; Linear third extract product tip **ce34b29** / PR #129 (t_7d3260b2 `ensure_linear_refusal_checkpoint` → `linear_issue_stages.sh`, 613→495 / stages 357→483) + docs tip-sync #130 @ **b97fecc**; Linear fourth extract product tip **48aa21b** / PR #131 (t_81b53e16 `ensure_linear_checkpoint_stage` + `ensure_linear_implement_in_progress` → `linear_issue_stages.sh`, 495→386 / stages 483→599) + docs tip-sync #132 @ **3e7cbc5**; Linear fifth/final thin sequencer product tip **f6b34d4** / PR #133 (t_d9eb82bb thin `process_linear_issue` sequencer, 386→329 / stages 599→725; Linear process_linear_issue thin sequencer complete like GitHub a3d9702) + docs tip-sync #134 @ **5418159**; shared helpers first extract product tip **d04f5e9** / PR #136 (test-write) + docs tip-sync #137 @ **5e53aaf**; shared helpers second extract product tip **593e18b** / PR #138 (t_9e60aed0: line-limit + ensure_publishable → `bin/lib/issue_shared.sh`, `bin/grkr` 339→279 / issue_shared 110→178) + docs tip-sync #139 @ **ce37e6a**; shared helpers third extract product tip **325ee9a** / PR #140 (t_2091085e: run_codex_prompt → `bin/lib/issue_shared.sh`, `bin/grkr` 279→260 / issue_shared 178→198) + docs tip-sync #141 @ **36de1a1**; shared helpers fourth extract product tip **f6fb872** / PR #142 (t_bc6ebfda: run_progress_cli + checkpoint_marker → `bin/lib/issue_shared.sh`, grkr 260→233 / issue_shared 198→228) + docs tip-sync #143 @ **3b0b2df**; shared helpers fifth extract product tip **c801967** / PR #144 (t_2847ff4d: attach_issue_logs → `bin/lib/issue_shared.sh`, grkr 233→198 / issue_shared 228→249) + docs tip-sync #145 @ **9a1b8f6**; github_issue thinning design tip **15a5050** / PR #146; github_issue PR body helpers product tip **1216e94** / PR #147 (t_e06571e3: ensure_pr_body_limit + extract_codex_pr_body → Gleam; github_issue.sh 545→543 / templates 176→221) + docs tip-sync #148 @ **745ca83**; swappable coding agent product tip **d55dd73** / PR #149; coding-agent matrix + quality eval harness product tip **1edf636** / PR #150 + docs tip-sync #151 @ **c918cde**; github_issue completion summary polish product tip **29c7a4b** / PR #152 (t_dec62328: render_github_completion_summary → Gleam + thin post_completion_comment; github_issue.sh 543→542 / templates 221→238) + docs tip-sync #153 @ **4613b02**; supervisor phases LOC hygiene product tip **0d13a98** / PR #154 (t_94976f9c: phases.gleam 688→117 + 7 concern modules; zero behavior change) + docs tip-sync #155 @ **d02734b**; progress/main LOC hygiene product tip **71075c2** / PR #156 (t_8c7cd0a0: main.gleam 644→63 thin facade + checkpoint_plan/linear_plan/templates_cli/linear_apply; zero behavior change) + docs tip-sync #157 @ **a426932**; handle_comment LOC hygiene product tip **e358311** / PR #158 (t_a12fcdec: handle_comment.gleam 456→92 thin facade + 6 concern modules; zero behavior change) + docs tip-sync #159 @ **3216246**; supervisor/comment_handler LOC hygiene product tip **178c535** / PR #160 (t_a9bf1687: comment_handler.gleam 430→91 thin facade + 6 concern modules; zero behavior change, stable supervisor entry) + docs tip-sync #162 @ **9b36632**; resolve_pr/main LOC hygiene product tip **cfdfb76** / PR #164 + docs tip-sync #165 @ **ce5830c**; deploy Docker+Helm **deb0acc** / PR #166; linear_issue_stages stages-split design tip **6250788** / PR #167; linear_issue_stages stages-split slice 1 product tip **34ab0f8** / PR #168 + docs tip-sync #169 @ **55ee02b**; linear_issue_stages stages-split slice 2 product tip **acd2910** / PR #170 (t_5ed4be3d: `ensure_linear_test_checkpoint` → `linear_issue_stages_test.sh`; facade sources refusal+test; stages ~623→~472 + test ~176); slice 3 publish extract product tip **feed4ea** / PR #173 (t_e0c51a35: `ensure_linear_publish_complete` → `linear_issue_stages_publish.sh`; facade sources refusal+test+publish; stages ~472→~323 + publish ~175); linear_issue_stages stages-split slice 4 product tip PR #175 @ **249dbbd** (t_952ad862) + docs tip-sync #176 @ **f25df23**; linear_issue_stages stages-split slice 5 product tip PR #177 @ **cb6b1b5** (t_ffc321af: `run_linear_decision_stage` + `handle_linear_decision_refuse` + `run_linear_implement_stage` → `linear_issue_stages_implement.sh`; facade source-only; stages ~225→~88 + implement ~133; stages-split complete); lineage tip-sync #174 @ **6eb24bf** after slice 3 **feed4ea** / #173); GitHub stages-split slice 1 product tip **6ef9f1d** / PR #181 (t_f8ceeffc: research/plan cluster → `github_issue_stages_research_plan.sh`; facade sources research_plan; `github_issue.sh` 542→459 + research_plan 111; design **d51ae73** / #180); GitHub stages-split slice 2 product tip **b19a5ba** / PR #183 (t_be3ee540: test → `github_issue_stages_test.sh`; facade sources research_plan then test; `github_issue.sh` 459→332 + test 154); GitHub stages-split slice 3 product tip **b4ecc04** / PR #186 (t_f2a425d1: publish+completion → `github_issue_stages_publish.sh`; facade sources research_plan then test then publish; `github_issue.sh` 332→210 + publish 151); GitHub stages-split slice 4 product tip **6dc13ac** / PR #189 (t_3fdd43d1: bootstrap/decision/refuse/implement/finalize → `github_issue_stages_implement.sh`; facade source-only research_plan→implement→test→publish; `github_issue.sh` 210→71 + implement 173; research_plan 111 + test 154 + publish 152; stages-split **complete**) + docs tip-sync #190 @ **c0d4d5d**; issue_shared concern-split design **a594167** / PR #191; progress/linear_mutation LOC hygiene product tip **7053645** / PR #192 (t_d0a2d481: 440→44 facade + types 29 + build 117 + parse 183 + policy 148) + docs tip-sync #193 @ **d706ebd**; issue_shared concern-split slice 1 product tip **1dca874** / PR #195 (t_c405d9ff: attach → `issue_shared_attach.sh`; facade sources attach) + docs tip-sync #196 @ **bb07d51**; issue_shared concern-split slice 2 product tip **7b4ac5e** / PR #197 (t_28a87184: progress → `issue_shared_progress.sh`; facade sources attach then progress) + docs tip-sync #198 @ **6f199d6**; issue_shared concern-split slice 3 product tip **f9ef4ed** / PR #199 (t_8ecf03f2: line-limit → `issue_shared_line_limit.sh`) + docs tip-sync #200 @ **3d06938**; slice 4 product tip **4b71cf1** / PR #201 + docs tip-sync #202 @ **b2d140d**; slice 5 FINAL parent product tip **851bed2** / PR #203 (t_3d89a6b9: coding-agent → `issue_shared_coding_agent.sh`; facade source-only; concern-split **complete**; design **a594167** / #191) + doctor/validate concern-split slice 2 product tip **88e3eea** / PR #220 (t_74a7a161: 387→67 thin facade + tools/agent/config/run; design **cbf74fc** / #217) + parent coding_agent concern-split slice 1 product tip **20310e2** / PR #218 (t_e7ea2b4b: 399→105 thin facade + types/select/classify/run; design **cbf74fc** / #217) + parent coding-agent polish P0+P1 product tip **b49a072** / PR #215 (t_49452ed3: doctor COMMENT/RESOLVE template + GROK_MODEL default grok-4.5; design **910ba27** / #214) + parent Gleam coding-agent swap slice 2 product tip **c55f8e1** / PR #212 (t_beed04ab; design **a87f379** / #207) + parent slice 1 **4553356** / PR #210 (t_45efd0d1) + docs tip-sync #211 @ **f3c4d40** after #210 + docs tip-sync #208 @ **71e329d** after docs tip **caef425** / PR #206 (checkpoint-json Gleam helpers **NO-GO**) + lineage docs #205 @ **b3c614f** (coding-agent matrix eval refresh) + tip-sync #204 @ **48f25e9**; see gleam-migration.md):**- github_picker (client+main+picker + decoder_test 256/256 green post fixtures fix + hygiene M in client/decoder/field t_64f72de6 + t_077f26d0 (0 warnings fix for field/client/decoder.gleam)), refusal (flow/assessment/checkpoint + cli; assessment WIP t_639e3cb9), supervisor (main/loop/recovery/state/lock/config/phases split t_94976f9c: thin 117 + log/sync/pick/reap/cleanup/scan_pr/scan_comment all <<200 + pick.gleam 152LOC dispatching `GRKR_ISSUE_PROVIDER` github|linear + scheduler 130 + FFI + comment_handler LOC split landed @ **178c535** / PR #160 (t_a9bf1687: 430→91+6 modules) implemented + reviewed in slices; phases pick via `supervisor/pick` + sync/scan_pr/scan_comment/reap/cleanup (LOC hygiene per design §6; build/test green, zero behavior change); progress/main split hygiene (644→63 facade + checkpoint_plan/linear_plan/templates_cli/linear_apply) landed @ **71075c2** / PR #156 (zero behavior change); handle_comment LOC hygiene (456→92 facade + 6 concern modules) landed @ **e358311** / PR #158 (zero behavior change); supervisor/comment_handler LOC hygiene (430→91 facade + 6 concern modules) landed @ **178c535** / PR #160 (zero behavior change)
- workflow/ (decision 270, decision_gate 155 (spec/22), implement_stage 36 + test, test_stage 66 LOC (run-tests + completion-marker per spec/26+39), handle_comment 92 (thin facade) + handle_comment_* (types 23, context 129, reactions 48, worktree 88, codex 87, result 40) LOC hygiene split landed @ **e358311** / PR #158 (t_a12fcdec), resolve_pr/main 81 (thin facade) + workflow 141 + apply 175 + runtime 108 (LOC hygiene landed @ **cfdfb76** / PR #164, t_681d0503), task_log split, worktree split, main/ffi); note supervisor/comment_handler split is parallel hygiene (not workflow/)
- Bin updates/hygiene (per AGENTS: preserve sh conv, small explicit changes, <1000 LOC): **doctor.sh (51 LOC thin: doctor_init + delegate to `grkr/doctor/cli`)**, grkr-project-status.sh (81 LOC thin), grkr-issue-workflow.sh (80 LOC thin wrapper delegating workflow/* CLIs incl decision_gate + test_completion_marker), worker-handle-comment.sh (29 LOC thin), worker-pick-issue.sh (46 LOC), worker-sync-main.sh (18 LOC), worker-resolve-pr.sh (39 LOC), worker-refuse-issue.sh (40 LOC thin calling refusal/cli), robot-main.sh (57 LOC), grkr (now thinner post-extract), grkr-templates.sh (62 LOC thin), + bin/lib/{refusal_paths.sh,task_progress.sh,issue_shared.sh,github_issue.sh,github_issue_stages_research_plan.sh,github_issue_stages_implement.sh,github_issue_stages_test.sh,github_issue_stages_publish.sh,linear_issue_stages.sh,linear_issue_stages_refusal.sh,linear_issue_stages_research_plan.sh,linear_issue_stages_implement.sh,linear_issue_stages_test.sh,linear_issue_stages_publish.sh} (github_issue.sh facade source-only research_plan→implement→test→publish siblings (stages-split slices 1–4 **complete**; design **d51ae73** / #180; product tip **6dc13ac** / #189 + docs tip-sync #190 @ **c0d4d5d**); progress/linear_mutation LOC hygiene product tip **7053645** / #192 (t_d0a2d481: 440→44 facade+concerns) + docs tip-sync #193 @ **d706ebd**; issue_shared concern-split design **a594167** / #191; issue_shared concern-split slice 1 product tip **1dca874** / #195 (t_c405d9ff: attach → `issue_shared_attach.sh`; facade sources attach fail-closed) + docs tip-sync #196 @ **bb07d51**; issue_shared concern-split slice 2 product tip **7b4ac5e** / #197 (t_28a87184: progress → `issue_shared_progress.sh`; facade sources attach then progress) + docs tip-sync #198 @ **6f199d6**; issue_shared concern-split slice 3 product tip **f9ef4ed** / PR #199 + docs tip-sync #200 @ **3d06938**; slice 4 product tip **4b71cf1** / PR #201 + docs tip-sync #202 @ **b2d140d**; slice 5 FINAL parent product tip **851bed2** / PR #203 (t_3d89a6b9: coding-agent → `issue_shared_coding_agent.sh`; facade source-only coding_agent→progress→test_write→line_limit→attach; concern-split **complete**) + doctor/validate concern-split slice 2 product tip **88e3eea** / PR #220 (t_74a7a161: 387→67 thin facade + tools/agent/config/run; design **cbf74fc** / #217) + parent coding_agent concern-split slice 1 product tip **20310e2** / PR #218 (t_e7ea2b4b: 399→105 thin facade + types/select/classify/run; design **cbf74fc** / #217) + parent coding-agent polish P0+P1 product tip **b49a072** / PR #215 (t_49452ed3: doctor COMMENT/RESOLVE template + GROK_MODEL default grok-4.5; design **910ba27** / #214) + parent Gleam coding-agent swap slice 2 product tip **c55f8e1** / PR #212 (t_beed04ab) + parent slice 1 **4553356** / PR #210 (t_45efd0d1) + docs tip-sync #211 @ **f3c4d40** after #210 + docs tip-sync #208 @ **71e329d** after docs tip **caef425** / PR #206 (checkpoint-json Gleam helpers **NO-GO**) + lineage docs #205 @ **b3c614f** (coding-agent matrix eval refresh) + tip-sync #204 @ **48f25e9**; linear_issue.sh sources linear_mutate + now linear_issue_stages; first shared extract product tip **d04f5e9** / PR #136 (t_d9c18700: test-write cluster → `bin/lib/issue_shared.sh`) + docs tip-sync #137 @ **5e53aaf**; second shared extract product tip **593e18b** / PR #138 (t_9e60aed0: line-limit + ensure_publishable_file_sizes → issue_shared.sh, `bin/grkr` 339→279 / issue_shared 110→178; design **d90fbaf** / PR #135) + docs tip-sync #139 @ **ce37e6a**; third shared extract product tip **325ee9a** / PR #140 (t_2091085e: run_codex_prompt (codex/exec + persist bridge) → issue_shared.sh, `bin/grkr` 279→260 / issue_shared 178→198) + docs tip-sync #141 @ **36de1a1**; fourth shared extract product tip **f6fb872** / PR #142 (t_bc6ebfda: run_progress_cli + checkpoint_marker → issue_shared.sh, `bin/grkr` 260→233 / issue_shared 198→228) + docs tip-sync #143 @ **3b0b2df**; fifth shared extract product tip **c801967** / PR #144 (t_2847ff4d: attach_issue_logs verbatim to issue_shared.sh (remove from grkr, no wrapper; hygiene: summarize_text deleted as 0 callers)) + docs tip-sync #145 @ **9a1b8f6**; github_issue PR body helpers product tip **1216e94** / PR #147 (t_e06571e3: ensure_pr_body_limit + extract_codex_pr_body → Gleam progress/templates; github_issue.sh 545→543; templates 176→221) + docs tip-sync #148 @ **745ca83**; swappable coding agent **d55dd73** / PR #149 + coding-agent matrix **1edf636** / PR #150 (issue_shared gains coding-agent bridge + per-step matrix) + docs tip-sync #151 @ **c918cde**; github_issue completion summary polish **29c7a4b** / PR #152 (t_dec62328: render_github_completion_summary → Gleam + thin post_completion_comment; github_issue.sh 543→542; templates 221→238). grkr 435→339→279→260→233→198 LOC; issue_shared 110→178→198→228→249→387 LOC (thinner grkr; no remaining shared launcher-only helpers after slice 5). grkr now 198 LOC post attach_issue_logs extract; templates ~238 post completion polish.
- Fully migrated: **doctor** (config_parse + validate + cli), sync_main, resolve_pr (PR conflicts), issue_provider (Linear), progress (checkpoints/Linear + templates ~238 incl PR body helpers + completion summary), task_slug, project_status (full + 81 LOC thin bin/grkr-project-status.sh delegating to project_status_cli), linear e2e
...
# Gleam-only step overrides (shell issue path ignores these):
# GRKR_AGENT_COMMENT=codex
# GRKR_AGENT_RESOLVE=grok   # resolve_pr ConflictResolve (slice 2)
...
Put these in `.grkr/config.sh` or the environment. Doctor validates only the selected agent. Design: [`docs/design-swappable-coding-agent.md`](docs/design-swappable-coding-agent.md). Polish P0+P1 **landed** @ **b49a072** / #215 (design [`docs/design-coding-agent-polish.md`](docs/design-coding-agent-polish.md)). coding_agent concern-split slice 1 **landed** @ **20310e2** / #218 (t_e7ea2b4b: 399→105 thin facade + types/select/classify/run; zero behavior change; design **cbf74fc** / #217). doctor/validate concern-split slice 2 **landed** @ **88e3eea** / PR #220 (t_74a7a161: 387→67 thin facade + tools 71 + agent 110 + config 132 + run 103; zero behavior change). Both ordered GO slices from design **cbf74fc** / #217 landed; remaining optional later only — [`docs/design-next-product-after-coding-agent-polish.md`](docs/design-next-product-after-coding-agent-polish.md).
...
4. **Progress CLI**: `gleam run -m grkr/progress/cli -- marker <stage> <task-slug>` renders the checkpoint marker used by the shell workflow, so `bin/grkr` and `worker-refuse-issue.sh` now delegate marker production to Gleam. The same CLI also exposes safe Linear planning commands (`linear-state`, `linear-comment-mutation`, `linear-state-mutation`, `plan-linear-refusal`, `mutation-debug`, and `check-token`) that print planned mutations or token status without contacting Linear by default. Linear state mutations use stage-scoped idempotency keys (`grkr-state-<stage>-<id>`; legacy 2-arg yields `...-update-...`). Live apply (`GRKR_LINEAR_MUTATE=1`) treats `skipped-no-token` as soft (no blocking sidecar; resume-safe). Optional `GRKR_LINEAR_MUTATE_STRICT=1` makes non-idempotent apply failures hard (non-zero) from the apply helper; refuse dumps (basename `refusal.*`) remain soft; default (STRICT unset) is always soft. Linear refuse planning writes `refusal.md` + `refusal.linear-mutation.txt` (+ optional state mutation dry-run) and updates `progress.json` to refused/skipped-test parity with the GitHub path.
...
1. `robot-main.sh` (now a thin ~60-line wrapper) sources doctor + config, runs validation, exports `VALIDATION_OK`, and always `exec`s the Gleam supervisor at `src/grkr/supervisor/main` so startup validation failures are logged/gated by the supervisor instead of exiting before `.grkr/logs/loop.log` exists; the Gleam code creates the `.grkr` layout and runs the ordered phases on the configured interval
...
- `worker-refuse-issue.sh` handles the refusal flow for issues that should not be implemented yet; it generates `refusal.md` with class and reasoning, posts a refusal checkpoint comment exactly once without duplication on resume, moves the project item from `Todo` to `Backlog` when `ENABLE_PROJECT_STATUS_UPDATES` and `REFUSAL_REQUIRES_BACKLOG_MOVE` allow it (GitHub), marks the workflow as refused, and treats refusal as a valid terminal state. With `GRKR_ISSUE_PROVIDER=linear`, the same thin worker accepts Linear identifiers (e.g. `ENG-123`), plans Linear comment + Backlog state mutations via progress helpers (dry-run by default; no `gh` backlog move), and emits `PROVIDER=linear` shell KEY=val lines.
- `workflow/decision_gate` refuse branch is provider-aware: GitHub keeps numeric issue + `refusal/flow`; Linear uses `refusal/linear_flow` + planned mutations (same dry-run posture as research/plan MVP).
...
- `grkr --linear-issue <identifier>` loads Linear context without `gh issue view`, writes research/plan checkpoints, plans Linear comment mutations, prepares an isolated worktree, runs the provider-aware decision gate, on proceed runs implement codex writing `implementation.log` + plans "In Progress" state mutation (dry-run), then test stage (commands + test.md + "In Review"), then publish+complete: stages, commits with linear msg ("feat(robot): implement ENG-123 ..."), pushes linear-* branch, creates/updates GitHub PR (body has "Linear: ENG-123" + url, no Fixes footer, no GitHub issue labels), calls mark complete (status=complete + branch/pr urls), plans complete.linear-state-mutation.txt (Done) + complete.linear-mutation.txt (summary + links) under dry-run. STAGE=complete on success. GitHub default untouched.
...
For fixture-backed selection, set `LINEAR_FIXTURE_PATH` to a JSON file containing Linear API response data. Selection emits shell-safe Linear issue metadata (`ISSUE_IDENTIFIER`, title, URL, state, priority, update time, job key `linear:<ID>:execution`, and task slug). The supervisor schedules Linear work via `JobKey.LinearExecution`, records `entity_type=issue_linear` in `active_jobs.json` with lock names in `eng-123` style, and spawns `grkr --linear-issue <identifier>`. The GitHub path (`grkr --issue N`, `entity_type=issue`) is unchanged. The `grkr --linear-issue <identifier>` workflow loads Linear issue context (fixture via `LINEAR_FIXTURE_PATH` or live GraphQL via `GRKR_LINEAR_ACCESS_TOKEN` / `~/.linear/token.txt`), writes research+plan checkpoints under `.grkr/tasks/<task_slug>/`, plans `progress/cli linear-comment-mutation` payloads (dry-run default; `GRKR_LINEAR_MUTATE=1` enables guarded live apply), prepares a `linear-<slug>` worktree, invokes the provider-aware decision gate (reusing `linear_flow` on refuse), and on proceed runs implement codex (`implementation.log` + In Progress dry-run), test stage (test.md + In Review dry-run), then publish+complete (GitHub PR from linear-* branch with Linear: ID footer, mark complete, Done state + completion comment dry-run dumps; STAGE=complete). Fetch-issue → shell uses single-line `KEY="value"` assignments with `\n`/`\r` escapes so multi-line Linear titles/descriptions survive into `issue-context.json` and research checkpoints. Linear **refuse** (t_e47417cb + t_503ca0f3) is wired through `worker-refuse-issue.sh` / `refusal/cli` / `decision_gate` when `GRKR_ISSUE_PROVIDER=linear`: plans commentCreate + Backlog state via `plan-linear-refusal` / `refusal/linear_flow`, writes `refusal.md` + progress refuse parity, no `gh` project moves. Guarded live apply is wired (`GRKR_LINEAR_MUTATE=1` literal enables; default OFF, soft-fail, sidecars at *.linear-apply-result.txt, stage-scoped state keys). GitHub remains default `GRKR_ISSUE_PROVIDER`.
