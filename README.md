...
5. Before it starts implementation, the issue helper creates `.grkr/tasks/<issue-slug>/`, writes `research.md`, `plan.md`, and `progress.json`, and posts the research and plan checkpoints back to the issue
6. If matching checkpoint comments already exist, the issue helper reuses those comments and resumes without reposting duplicate research or plan checkpoints
7. Before it checks out or creates `issue-N`, the issue helper moves the configured project item to `In Progress` when it can resolve that project item
8. The issue helper reuses branch `issue-N` when it already exists locally or remotely, otherwise creates it
9. After Codex finishes implementing, `grkr` posts the local run log back to the issue inside a collapsed details block
10. `grkr` then commits, pushes, and opens a PR that links the issue
11. If the generated PR description is too large for GitHub, `grkr` replaces it with a compact summary before creating the PR
