...
Current implementation status: see [docs/gleam-migration.md](./docs/gleam-migration.md) for v2 Gleam migration progress (github_picker/refusal/supervisor **GitHub** pick+schedule+spawn complete; Linear supervisor schedule+spawn via `JobKey.LinearExecution` / `grkr --linear-issue` landed t_51747d23 @ **ce61881**; fixture e2e verified t_5254b489; `--linear-issue` research/plan MVP (t_48db8096) with multi-line KEY=val wire protocol (`
` escapes) @ **300** `gleam test` on branch tip; **298** on **main** pre-MVP; PR #79 **MERGED** 2026-07-10; implement/test/PR + refusal still follow-up; GitHub remains default `GRKR_ISSUE_PROVIDER`).
...
- Remaining work (Linear implement/test/PR + refusal/state mutations after `--linear-issue` research/plan MVP; supervisor Linear pick→schedule→spawn already landed; GitHub default; spec/39 items 6–12 implemented; see gleam-migration.md table)
...
**High-level snapshot (@ post-merge tip including `ce61881` Linear JobKey+spawn + t_5254b489 fixture e2e + `--linear-issue` research/plan MVP t_48db8096; includes PR #79 merge `78f4fb8`; pre-merge tip `62c73fb`; hygiene + worker-help + robot-main validation-failure + npm-order flake isolation; **300** gleam on MVP tip / **298** on main pre-MVP + schedule fixture green; GitHub path complete; Linear schedule/spawn on main; research/plan MVP landed; implement/test/PR + refusal still follow-up; see gleam-migration.md):**
