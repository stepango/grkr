...
Current implementation status: see [docs/gleam-migration.md](./docs/gleam-migration.md) for v2 Gleam migration progress (github_picker/refusal/supervisor **GitHub** pick+schedule+spawn complete @ **295** `gleam test` / `e894354`; latest WIP t_639e3cb9 + robot-main validation-failure fix + npm-order flake isolation + docs hygiene t_8899beba/t_48e8e864/t_df49c38c/t_4f91bfcd: supervisor phases/exec, refusal assessment, workflow decision; Linear full execution still forward-looking — t_51747d23 deferred; `supervisor/pick.gleam` + `GRKR_ISSUE_PROVIDER` per t_73c1fbdf).
