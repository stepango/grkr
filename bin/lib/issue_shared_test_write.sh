# bin/lib/issue_shared_test_write.sh
# Concern-split slice 4 (docs/design-issue-shared-concern-split.md):
# build_command_list + cleanup_test_result_logs + write_test_checkpoint_with_header
# extracted from issue_shared.sh.
# Facade (issue_shared.sh) sources this sibling; bin/grkr still sources only the facade.
# Ambient call-time deps: checkpoint_marker from progress sibling (call-time
# resolution OK after full facade source order: progress before test_write).
# Zero behavior change; stable public names build_command_list,
# cleanup_test_result_logs, write_test_checkpoint_with_header.
