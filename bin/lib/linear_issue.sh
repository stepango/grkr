...
# Linear --linear-issue helpers for bin/grkr (research + plan + decision + implement + test dry-run).
...
# No GitHub gh issue view / pr create. Publish + complete deferred. GitHub default untouched.
...
process_linear_issue() {
  ...
  # Success path continuation: after implement success, run test stage (exec in worktree,
  # write test.md with Linear header, plan dry-run comment + "In Review" state mutations).
  # GitHub path untouched. No publish, no mark_complete. Leave worktree on success.
  ensure_linear_test_checkpoint \
    "$ISSUE_IDENTIFIER" "$mutation_issue_id" "$TASK_SLUG" "$TASK_DIR" \
