...
**Current implementation status (2026-07-12, post-merge tip **`28e4794`** = PR #93 Linear MVP after #92 docs): PR #79 **MERGED** to `main` @ **78f4fb8** (mergedAt 2026-07-10T23:46:23Z; pre-merge tip `62c73fb`). GitHub picker/refusal/supervisor pick+schedule+spawn path is on **main**; Linear JobKey + `spawn_linear_issue_execution` landed (t_51747d23 @ **ce61881**: `linear:ID:execution`, entity_type `issue_linear`, locks eng-123, `grkr --linear-issue`); fixture e2e + docs sync t_5254b489 / PR #92 @ **5782431** (**298** gleam / npm green on main pre-MVP); workflow CLI research+plan MVP via `--linear-issue` (t_48db8096 / PR #93 @ **28e4794**) with multi-line KEY=val wire protocol now on **main**; implement/test/PR deferred; Linear refuse/state still open (t_503ca0f3 + t_e47417cb); unified `supervisor/pick.gleam` dispatches `GRKR_ISSUE_PROVIDER` github|linear for pick+schedule; refusal-protected worktree cleanup + active_jobs TTL/hung-lock recovery per `.grkr/supervisor-cleanup-policy.md` §6; `gleam build` clean; GitHub-default on **main** (PR #79 closed/merged). Module inventory (live wc/find): github_picker/ (10 modules +5 mjs ~1239 LOC), refusal/ (7+4mjs ~1084 LOC), supervisor/ (12+7mjs ~2300 LOC: phases 688, pick 152, loop 255, state 263, recovery 214, types 181, config 163, scheduler 130, ffi 125, lock 88, main 56, comment_handler 37), workflow/ (14+3mjs ~1471 LOC: decision 270, decision_gate 155 (spec/22 implement-or-refuse gate + CLI + refusal integration), implement_stage 36, handle_comment 61, resolve_pr/main 426 (full), task_log split (core 187 + persist 113 + cli 83 + task_log 41 + types 7), worktree split (ops 146 + stage 59 + worktree 45 + worktree_types 10), main 77, ffi 75), progress/ (8+1mjs ~1152 LOC: main 325, templates 176, linear_mutation 174, cli 124, checkpoint_render 103, linear_state 112, checkpoint_id 74, checkpoint_stage 64), issue_provider/ (10 ~1663 LOC: main 236, config 269, decoder 225, types 222, query 200, selector 150, credential 182, validation 88, client 65, ffi 26), resolve_pr/ (5 ~968 LOC: main 426 full, git 240, codex 134, github 121, types 47; workflow/resolve_pr.gleam skeleton ref), linear/ (7 ~1182 LOC: oauth 353, e2e 272, client 209, config 147, graphql 106, types 75, e2e_main 20), project_status/ (7 ~1198 LOC), task_slug/ (2 ~134 LOC), doctor/ (4 ~559 LOC) + thin bin wrappers; shell wrappers preserve external contracts.
...
- Linear **workflow implement/test/PR** after `--linear-issue` research/plan MVP (t_48db8096 / PR #93 @ **28e4794** landed: research+plan checkpoints, multi-line KEY=val wire protocol, dry-run linear-comment-mutation plans); Linear refusal + state mutations still open — **t_503ca0f3** + **t_e47417cb**. Supervisor Linear pick→schedule→spawn already landed (t_51747d23 @ ce61881; fixture e2e t_5254b489 / PR #92 @ 5782431). GitHub remains default `GRKR_ISSUE_PROVIDER`.
