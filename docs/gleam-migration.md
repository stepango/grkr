...
- Linear **workflow implement/test/PR** after `--linear-issue` research/plan MVP (t_48db8096 / PR #93 @ **28e4794** landed: research+plan checkpoints, multi-line KEY=val wire protocol, dry-run linear-comment-mutation plans). Linear **refuse wire** (t_e47417cb + progress planning t_503ca0f3: `plan_linear_refusal` + `ensure_linear_refusal_checkpoint` + `refusal/linear_flow` + provider-aware `refusal/cli` / `decision_gate` / `worker-refuse-issue.sh`, progress.json refuse parity, no live GraphQL by default) landed; live `GRKR_LINEAR_MUTATE` apply still optional follow-up. Supervisor Linear pick→schedule→spawn already landed (t_51747d23 @ ce61881; fixture e2e t_5254b489 / PR #92 @ 5782431). GitHub remains default `GRKR_ISSUE_PROVIDER`.
- **PR #79 MERGED** to `main` @ 78f4fb8 (2026-07-10); Linear JobKey @ **ce61881**; main tip **28e4794** (PR #93); ongoing e2e cron regression + kanban hygiene (process — not missing spec/39 core modules)
