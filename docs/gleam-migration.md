...
**Update t_28a87184 (issue_shared concern-split slice 2: progress extract):** Functional shell extraction (zero intentional behavior change). Moved exact bodies of `run_progress_cli` + `checkpoint_marker` from `bin/lib/issue_shared.sh` into `bin/lib/issue_shared_progress.sh`. Facade BASH_SOURCE-sources attach then progress (fail-closed). Remaining bodies: test-write, line-limit, coding-agent. `bin/grkr` still sources only facade. Design **a594167** / #191; parent tip **1dca874** / #195 + tip-sync #196 @ **bb07d51**.
