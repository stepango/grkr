...
- **Next product thinning (design only):** GitHub `process_issue` / thick `bin/grkr` (~875 LOC) → Gleam/thin lib slices — see [`docs/design-github-process-issue-thinning.md`](design-github-process-issue-thinning.md) (t_dabc33a8). First implement: extract GitHub test checkpoint to `bin/lib/github_issue.sh`. Linear `linear_issue.sh` ~923 LOC near 1000 — extract shared helpers before any Linear growth.
