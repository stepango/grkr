...
**Current implementation status (2026-07-14, post publish+complete @ bd523a6 / PR #100; main tip 7f0fafc): GitHub picker/refusal/supervisor on **main**; Linear research/plan + refuse + implement + test + publish+complete wired for `--linear-issue` (STAGE=complete; GitHub PR from linear-* + Done + comment); guarded live mutate (`GRKR_LINEAR_MUTATE=1` default-OFF, dumps+sidecars, stage keys) landed; GitHub default; `gleam build` clean. (See full table + remaining below.)**
...
- Linear publish + complete @ **bd523a6** (PR #100) + guarded live mutate apply: after test, produces GitHub PR from linear-* + plans/applies (when enabled) Linear Done + completion comment. Guarded apply (`GRKR_LINEAR_MUTATE=1` literal enables; default OFF, soft-fail, *.linear-apply-result.txt sidecars, stage-scoped grkr-state-<stage>-<id> keys, COMMENT FIRST then state for complete). No live in default tests/CI. Supervisor Linear pick→schedule→spawn already landed. GitHub remains default `GRKR_ISSUE_PROVIDER`.
