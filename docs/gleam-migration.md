...
**Current implementation status (2026-07-16, product tip lineage @ **a3d9702** / PR #121 thin process_issue orchestrator (t_3619188b) after completion **def63d8** / PR #119 + research/plan **53592d4** / PR #117 + publish-helpers **6e0f1d3** / PR #115 + test-checkpoint **c438409** / PR #112; `bin/lib/github_issue.sh`, `bin/grkr` 875→...→527→435; docs tip-sync #120 @ **321aa11**; STRICT @ **82f3644** / PR #110; nits @ **8d4b674** / PR #107; land @ **e28d0c0** / PR #104; publish+complete @ bd523a6 / PR #100; FFI variables fix @ **36f8f03** / PR #106; docs tip-sync #111 @ **f07d578**; docs tip-sync #113 @ **219fde5**; docs tip-sync #116 @ **2ac57fb**; docs tip-sync #118 @ **bf530e5**): GitHub picker/refusal/supervisor on **main**; Linear research/plan + refuse + implement + test + publish+complete wired for `--linear-issue` (STAGE=complete; GitHub PR from linear-* + Done + comment); guarded live mutate (`GRKR_LINEAR_MUTATE=1` default-OFF, dumps+sidecars, stage-scoped keys, stricter parse, soft skipped-no-token; optional `GRKR_LINEAR_MUTATE_STRICT=1` hard-fails non-idempotent non-refuse applies, refuse stays soft) **landed** @ **e28d0c0** + nits **8d4b674** + STRICT **82f3644**; thinning tip **a3d9702** (process_issue thin orchestrator + prior completion/research/plan/publish/checkpoint); GitHub default; `gleam build` clean. (See full table + remaining below.)**
...
- **PR #79 MERGED** to `main` @ 78f4fb8 (2026-07-10); Linear JobKey @ **ce61881**; MVP tip **28e4794** (PR #93); refuse tip **8aba009** (PR #95); implement tip **d1c1240** (PR #97); test tip **bfee58c** (PR #98); publish tip **bd523a6** (PR #100); live-mutate land **e28d0c0** (PR #104); Linear variables FFI **36f8f03** (PR #106); live-mutate nits **8d4b674** (PR #107); optional STRICT **82f3644** (PR #110); docs tip-sync #111 @ **f07d578**; test-checkpoint thinning **c438409** (PR #112); docs tip-sync #113 @ **219fde5**; publish-helpers thinning **6e0f1d3** (PR #115); docs tip-sync #116 @ **2ac57fb**; research/plan thinning **53592d4** (PR #117); docs tip-sync #118 @ **bf530e5**; completion surface thinning **def63d8** (PR #119; grkr 545→527); docs tip-sync #120 @ **321aa11**; process_issue thin orchestrator tip **a3d9702** (PR #121 / t_3619188b; `bin/grkr` 527→435; github_issue.sh 387→539); ongoing e2e cron regression + kanban hygiene (process — not missing spec/39 core modules)
- **Next product thinning:** GitHub `process_issue` / thick `bin/grkr` → thin lib slices (see [`docs/design-github-process-issue-thinning.md`](design-github-process-issue-thinning.md)). Slice 1 (PR #112 @ c438409): GitHub test checkpoint extracted to `bin/lib/github_issue.sh`. Slice 3 (PR #115 @ **6e0f1d3** / t_bb4a09e6): GitHub publish helpers (`publish_issue_changes`, `extract_codex_pr_body`, `ensure_pr_body_limit` with Fixes #N footer) extracted to same lib; `bin/grkr` calls them (net thinner 738→639 LOC); Slice 4 (PR #117 @ **53592d4** / t_b61f10e8): research/plan `ensure_checkpoint_stage` + gh comment helpers (`fetch_issue_comments_json` etc) extracted (grkr 639→545); Slice 5 (PR #119 @ **def63d8** / t_d328b158): completion surface (`post_completion_comment`) extracted (`bin/grkr` 545→527; github_issue.sh 360→387); Slice 6 (PR #121 @ **a3d9702** / t_3619188b): thin `process_issue` orchestration — bootstrap_github_issue_task + run_github_decision_stage + handle_github_decision_refuse + run_github_implement_stage + finalize_github_issue_complete moved to `bin/lib/github_issue.sh`; `bin/grkr` now thin sequencer/launcher (527→435 LOC); github_issue.sh 387→539; shared ensure_publishable_file_sizes stays in grkr; `linear_issue.sh` untouched (empty diff, 923 LOC); GitHub `--issue` contract 100% identical (zero behavior change). All regressions green. Product tip **a3d9702** / PR #121: process_issue is now clear launcher/orchestrator. No further GitHub shell thinning in this lineage. Linear `linear_issue.sh` ~923 LOC near 1000 — extract shared before growth.
