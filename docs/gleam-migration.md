...
- Linear publish + complete @ **bd523a6** (PR #100) + guarded live mutate apply **landed** @ **e28d0c0** (PR #104) + post-land nits tip **8d4b674** (PR #107): after test, produces GitHub PR from linear-* + plans/applies (when enabled) Linear Done + completion comment. Guarded apply (`GRKR_LINEAR_MUTATE=1` literal enables; default OFF, soft-fail; optional STRICT=1 hard-fails non-idempotent apply on non-refuse dumps (refuse.* always soft); *.linear-apply-result.txt sidecars, stage-scoped grkr-state-<stage>-<id> keys only, stricter `mutation_result` parse, soft resume-safe `skipped-no-token`, COMMENT FIRST then state for complete). No live in default tests/CI. Supervisor Linear pick→schedule→spawn already landed. GitHub remains default `GRKR_ISSUE_PROVIDER`.
- **PR #79 MERGED** to `main` @ 78f4fb8 (2026-07-10); Linear JobKey @ **ce61881**; MVP tip **28e4794** (PR #93); refuse tip **8aba009** (PR #95); implement tip **d1c1240** (PR #97); test tip **bfee58c** (PR #98); publish tip **bd523a6** (PR #100); live-mutate land **e28d0c0** (PR #104); Linear variables FFI **36f8f03** (PR #106); live-mutate nits tip **8d4b674** (PR #107); ongoing e2e cron regression + kanban hygiene (process — not missing spec/39 core modules)
