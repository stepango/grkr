...
**Current implementation status (2026-07-13, Linear implement dry-run slice after PR #95 refuse @ 8aba009 / #93 MVP): PR #79 **MERGED** to `main` @ **78f4fb8**. GitHub picker/refusal/supervisor on **main**; Linear JobKey + spawn landed; research/plan MVP + refuse dry-run + implement decision_gate + codex implement + In Progress state plan (dry-run, worktree + implementation.log + progress parity) wired for `--linear-issue`; test stage + publish deferred; GitHub default; `gleam build` clean. (Module inventory truncated in header; see full table below.)**
...
**Update (2026-07-13, Linear implement dry-run slice):** Product change. `--linear-issue` now drives decision_gate (provider-aware), implement codex → implementation.log, In Progress mutation planning (dry-run via ensure_linear_implement_in_progress + linear-state-mutation), progress parity, and impl-refusal conversion reusing linear_flow. GitHub path untouched. New test + updated mvp expectations. implement_stage now supports Linear commit msg form. README + gleam-migration updated. No live mutate; test/publish deferred. Ran full verification (build/test/shell).
