...
- Linear **workflow implement/test/PR** after `--linear-issue` research/plan MVP (t_48db8096 landed: research+plan checkpoints, multi-line KEY=val wire protocol, dry-run linear-comment-mutation plans); Linear refusal + state mutations — t_2315506a. Supervisor Linear pick→schedule→spawn already landed (t_51747d23 @ ce61881; fixture e2e t_5254b489). GitHub remains default `GRKR_ISSUE_PROVIDER`.
- **PR #79 MERGED** to `main` @ 78f4fb8 (2026-07-10); Linear JobKey tip **ce61881**; ongoing e2e cron regression + kanban hygiene (process — not missing spec/39 core modules)
