...
**Key Gleam modules implemented (all <1000 LOC, `gleam build` succeeds, `gleam test` 295 passed, 0 failures @ fe3fcd1):**
...
- **supervisor/** (loop orchestration, recovery, locking, state + phases; per design in supervisor-design-final.md; phases extraction + impl complete; loop updated):
  - main.gleam (56 LOC), phases.gleam (688 LOC), **pick.gleam (152 LOC, t_73c1fbdf)**: `issue_provider_name()` reads `GRKR_ISSUE_PROVIDER` (default `github`; `linear` → `issue_provider/main`); `pick_next()` → provider-agnostic `SelectedWork`; `schedule_selected()` spawns via scheduler when `issue_number` present (Linear identifier-only logs pending spawn follow-up), loop.gleam (~274 LOC), recovery.gleam (~336 LOC: dead PID + `recover_stale_active_jobs` TTL/hung-lock per policy §6), worktree_cleanup.gleam (~172 LOC: classify + `collect_refusal_protected_tokens`), config.gleam (~171 LOC, `ACTIVE_JOB_TTL_SECONDS`), types.gleam (181 + active_job_ttl_seconds), state.gleam (263), lock.gleam (88), ffi.gleam (~134), scheduler.gleam (130)
  - + process/exec/file/env/fs mjs; tests: `test/grkr/supervisor/pick_test.gleam` (fixture env: `GITHUB_FIXTURE_PATH`, `LINEAR_FIXTURE_PATH`, `GRKR_ACTIVE_JOBS_PATH`), `test/grkr/supervisor/scheduler_test.gleam` (GitHub `spawn_issue_execution`: active_jobs schema, flock lock + job log, `--issue` argv, `bin/grkr` fallback; t_09f442ca)
...
- Linear issue provider **full execution path** (experimental; discovery CLIs + opt-in e2e exist; GitHub remains default; kanban t_51747d23 deferred)
