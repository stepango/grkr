...
**Key Gleam modules implemented (all <1000 LOC, `gleam build` succeeds, `gleam test` 284 passed, 0 failures):**
...
- **supervisor/** (loop orchestration, recovery, locking, state + phases; per design in supervisor-design-final.md; phases extraction + impl complete; loop updated):
  - main.gleam (56 LOC), phases.gleam (~664 LOC), loop.gleam (~274 LOC), recovery.gleam (~336 LOC: dead PID + `recover_stale_active_jobs` TTL/hung-lock per policy §6), worktree_cleanup.gleam (~172 LOC: classify + `collect_refusal_protected_tokens`), config.gleam (~171 LOC, `ACTIVE_JOB_TTL_SECONDS`), types.gleam (181 + active_job_ttl_seconds), state.gleam (263), lock.gleam (88), ffi.gleam (~134), scheduler.gleam (130)
...
Verified **2026-06-21** (`gleam test`: **284 passed, 0 failures**; `gleam build` clean). Obsolete notes removed (decoder fixture failures, unused-import warnings — fixed in prior slices; github_picker decoder tests green).
