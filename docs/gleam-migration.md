...
**Current implementation status (2026-07-16, product tip @ **c438409** / PR #112 first GitHub process_issue thinning slice — test checkpoint → `bin/lib/github_issue.sh`, `bin/grkr` 875→738; STRICT @ **82f3644** / PR #110; nits @ **8d4b674** / PR #107; land @ **e28d0c0** / PR #104; publish+complete @ bd523a6 / PR #100; FFI variables fix @ **36f8f03** / PR #106; docs tip-sync #111 @ **f07d578**): GitHub picker/refusal/supervisor on **main**; Linear research/plan + refuse + implement + test + publish+complete wired for `--linear-issue` (STAGE=complete; GitHub PR from linear-* + Done + comment); guarded live mutate (`GRKR_LINEAR_MUTATE=1` default-OFF, dumps+sidecars, stage-scoped keys, stricter parse, soft skipped-no-token; optional `GRKR_LINEAR_MUTATE_STRICT=1` hard-fails non-idempotent non-refuse applies, refuse stays soft) **landed** @ **e28d0c0** + nits **8d4b674** + STRICT **82f3644**; thinning tip **c438409**; GitHub default; `gleam build` clean. (See full table + remaining below.)**
...
- **PR #79 MERGED** to `main` @ 78f4fb8 (2026-07-10); Linear JobKey @ **ce61881**; MVP tip **28e4794** (PR #93); refuse tip **8aba009** (PR #95); implement tip **d1c1240** (PR #97); test tip **bfee58c** (PR #98); publish tip **bd523a6** (PR #100); live-mutate land **e28d0c0** (PR #104); Linear variables FFI **36f8f03** (PR #106); live-mutate nits **8d4b674** (PR #107); optional STRICT **82f3644** (PR #110); docs tip-sync #111 @ **f07d578**; first GitHub process_issue thinning tip **c438409** (PR #112; `bin/lib/github_issue.sh`, `bin/grkr` 875→738); ongoing e2e cron regression + kanban hygiene (process — not missing spec/39 core modules)
- **Next product thinning:** GitHub `process_issue` / thick `bin/grkr` → thin lib slices (see [`docs/design-github-process-issue-thinning.md`](design-github-process-issue-thinning.md)). First slice landed: GitHub test checkpoint (`ensure_test_checkpoint` + `write_test_checkpoint_file`) extracted to `bin/lib/github_issue.sh`; `bin/grkr` sources it and is net thinner; shared `build_command_list` / `cleanup_test_result_logs` / `write_test_checkpoint_with_header` remain in grkr (used by Linear `ensure_linear_test_checkpoint`); `linear_issue.sh` untouched. GitHub contract bit-identical. See remaining slices in design. Linear `linear_issue.sh` ~923 LOC near 1000 — extract shared helpers before any Linear growth.
