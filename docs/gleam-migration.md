...
**Current implementation status (2026-07-13, post-merge tip **`8aba009`** = PR #95 Linear refuse dry-run wire after #93 MVP + #92 docs + t_503ca0f3/t_e47417cb): PR #79 **MERGED** to `main` @ **78f4fb8** (mergedAt 2026-07-10T23:46:23Z; pre-merge tip `62c73fb`). GitHub picker/refusal/supervisor pick+schedule+spawn path is on **main**; Linear JobKey + `spawn_linear_issue_execution` landed (t_51747d23 @ **ce61881**: `linear:ID:execution`, entity_type `issue_linear`, locks eng-123, `grkr --linear-issue`); fixture e2e + docs sync t_5254b489 / PR #92 @ **5782431** (**298** gleam / npm green on main pre-MVP); workflow CLI research+plan MVP via `--linear-issue` (t_48db8096 / PR #93 @ **28e4794**) with multi-line KEY=val wire protocol now on **main**; Linear refuse progress planning (t_503ca0f3: `progress/main.plan_linear_refusal`, CLI `plan-linear-refusal`, `bin/lib/linear_issue.sh ensure_linear_refusal_checkpoint`, `refusal/linear_flow` + provider-aware cli/decision_gate; dry-run comment+Backlog state mutations; progress.json refuse parity; **304** gleam + `test/grkr-linear-refuse-progress.sh`) landed; implement/test/PR deferred; unified `supervisor/pick.gleam` dispatches `GRKR_ISSUE_PROVIDER` github|linear for pick+schedule; refusal-protected worktree cleanup + active_jobs TTL/hung-lock recovery per `.grkr/supervisor-cleanup-policy.md` §6; `gleam build` clean; GitHub-default on **main** (PR #79 closed/merged). Module inventory (live wc/find): github_picker/ (10 modules +5 mjs ~1239 LOC), refusal/ (8+4mjs incl linear_flow ~1300 LOC), supervisor/ (12+7mjs ~2300 LOC: phases 688, pick 152, loop 255, state 263, recovery 214, types 181, config 163, scheduler 130, ffi 125, lock 88, main 56, comment_handler 37), workflow/ (14+3mjs ~1471 LOC: decision 270, decision_gate provider-aware refuse path, implement_stage 36, handle_comment 61, resolve_pr/main 426 (full), task_log split (core 187 + persist 113 + cli 83 + task_log 41 + types 7), worktree split (ops 146 + stage 59 + worktree 45 + worktree_types 10), main 77, ffi 75), progress/ (8+1mjs ~main with plan_linear_refusal, cli plan-linear-refusal, checkpoint_stage Refusal requires state update), issue_provider/ (10 ~1663 LOC: main 236, config 269, decoder 225, types 222, query 200, selector 150, credential 182, val... [truncated]
...
- Linear **workflow implement/test/PR** still deferred after `--linear-issue` research/plan MVP (t_48db8096 / PR #93 @ **28e4794** lineage: research+plan checkpoints, multi-line KEY=val wire protocol, dry-run linear-comment-mutation plans). Linear **refuse dry-run** (t_503ca0f3 + t_e47417cb land t_18df994e / PR #95 @ **8aba009**: `plan_linear_refusal` + `ensure_linear_refusal_checkpoint` + `refusal/linear_flow` + provider-aware `refusal/cli` / `decision_gate` / `worker-refuse-issue.sh`, progress.json refuse parity, no live GraphQL by default) **landed**; live `GRKR_LINEAR_MUTATE` apply still optional follow-up. Supervisor Linear pick→schedule→spawn already landed (t_51747d23 @ ce61881; fixture e2e t_5254b489 / PR #92 @ 5782431). GitHub remains default `GRKR_ISSUE_PROVIDER`.
- **PR #79 MERGED** to `main` @ 78f4fb8 (2026-07-10); Linear JobKey @ **ce61881**; MVP tip **28e4794** (PR #93); main tip **8aba009** (PR #95 refuse dry-run); ongoing e2e cron regression + kanban hygiene (process — not missing spec/39 core modules)
...
**Update t_d3e5eead (2026-07-13, post-land tip sync after Linear refuse PR #95):** Docs-only. Main tip promoted to **8aba009** (PR #95 Linear refuse dry-run wire: `plan_linear_refusal` + `refusal/linear_flow` + provider-aware decision_gate/worker-refuse; land card t_18df994e; parents t_503ca0f3 + t_e47417cb). README + this header + forward-looking bullets + `spec/parts/39` note refuse dry-run **landed**; implement/test/PR still deferred; GitHub default `GRKR_ISSUE_PROVIDER`. Ran `scripts/sync-spec.sh` after `spec/parts/39` touch. No product code.
