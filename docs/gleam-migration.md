...
- **Next product thinning:** both ordered GO slices from design **cbf74fc** / #217 **landed**: slice 1 `coding_agent` concern-split @ **20310e2** / #218 (t_e7ea2b4b: 399→105 thin facade + types/select/classify/run); slice 2 `doctor/validate` concern-split @ **88e3eea** / PR #220 (t_74a7a161: 387→67 thin facade + tools 71 + agent 110 + config 132 + run 103; zero behavior change; stable `grkr/doctor/validate`). Coding-agent polish P0+P1 **landed** @ **b49a072** / #215 (lineage). Spec/39 items 6–12 done; shell GitHub/Linear/issue_shared verticals complete; checkpoint-json **NO-GO**; artifact rename **NO-GO**; third backend **DEFER**. Remaining is **optional later only** (e.g. refusal backlog extract if touching that path); no inventing new GO slices or spec/39 features. (Historical thinning log retained in prior commits / Update bullets below.)
...
**Update t_3d89a6b9 (issue_shared concern-split slice 5 FINAL: coding-agent extract; facade source-only):** Functional shell extraction (zero intentional behavior change). Moved exact bodies of `_grkr_coding_agent_name` + `_grkr_coding_step_from_phase` + `_grkr_run_codex_backend` + `_grkr_run_grok_backend` + `run_coding_agent_prompt` + `run_codex_prompt` from `bin/lib/issue_shared.sh` into new sibling `bin/lib/issue_shared_coding_agent.sh`. Facade is source-only (design §4 order: coding_agent→progress→test_write→line_limit→attach fail-closed). `bin/grkr` still sources only facade. Log strings + env matrix (`GRKR_CODING_AGENT` / per-step overrides) byte-identical. Concern-split slices 1–5 **complete**. Remaining optional only was checkpoint-json low-ROI / pure Gleam coding-agent later (now checkpoint-json **NO-GO** (leave shell jq; see [`design-checkpoint-json-gleam.md`](design-checkpoint-json-gleam.md) / #206 @ **caef425**)). Design **a594167** / #191; parent tip **4b71cf1** / PR #201 + docs tip-sync #202 @ **b2d140d**. Kanban t_3d89a6b9. Merge tip **851bed2** / PR #203. **Update t_28e8e032 (tip-sync after #203):** Docs-only. Tip **851bed2** / #203. Concern-split complete. No product code.

**Update t_c6617364 (tip-sync after #206):** Docs-only. Docs tip **caef425** / #206 checkpoint-json **NO-GO**. Product tip still **851bed2** / #203. Lineage #205 @ **b3c614f**. No product code.

**Update t_45efd0d1 (Gleam coding-agent swap slice 1 — comment classify):** Product. New `src/grkr/coding_agent.gleam` + tiny `coding_agent_ffi.mjs`; wire both `handle_comment_codex` + `comment_handler_codex` to honor `GRKR_AGENT_COMMENT` → `GRKR_CODING_AGENT`/`CODING_AGENT` → codex; Grok classify via `--prompt-file` + temp cleanup; thread `worktree_info.dir`; unit tests `test/grkr/coding_agent_test.gleam` (fake exec). Default/codex argv+stdin+timeout+sandbox unchanged (no `--full-auto` on comment). resolve_pr still hardcoded (slice 2). Design **a87f379** / #207. `gleam test` 341 green. Merge tip **4553356** / PR #210. **Update t_ec2112da (tip-sync after #210):** Docs-only. Header product tip → **4553356** / #210. No product code.

**Update t_beed04ab (Gleam coding-agent swap slice 2 — resolve_pr ConflictResolve):** Product. Wire `resolve_pr/codex.gleam` through `coding_agent.run_with_defaults(Resolve, ConflictResolve, …)`; thread `worktree_path` from `apply.resolve_with_codex`; extend `resolve_pr/fs.mjs` with `temp_path`/`unlink_file` for Grok prompt file; generalize fail strings to `Coding agent`; unit tests for ConflictResolve argv (default codex `exec --full-auto` + prompt-as-argv + empty stdin; `GRKR_AGENT_RESOLVE=grok`; `CODEX_BIN`/`CODEX_ARGS`). Public API names unchanged. Design **a87f379** / #207. Slice 2 resolve_pr honors `GRKR_CODING_AGENT`. Merge tip **c55f8e1** / PR #212. **Update t_d6f2b7c9 (tip-sync after #212):** Docs-only. Header/status product tip → **c55f8e1** / #212 (t_beed04ab). Keep slice 1 #210 + design #207 in lineage. No product code.

**Update t_df8f9141 (coding-agent polish design):** Docs-only. New [`docs/design-coding-agent-polish.md`](design-coding-agent-polish.md): artifact path rename **NO-GO**; third backend **DEFER**; doctor COMMENT/RESOLVE template **GO** (P0); `GROK_MODEL` default → `grok-4.5` shell+Gleam **GO** (P1, paired). Parent follow-ups updated. No product code.
**Update t_49452ed3 (coding-agent polish P0+P1):** Product. Doctor `default_config_template` documents `GRKR_AGENT_COMMENT`/`GRKR_AGENT_RESOLVE`; shell + Gleam default `GROK_MODEL` → `grok-4.5` (explicit `grok-build` override still ok). Design **910ba27** / #214. `gleam test` 347 green. Merge tip **b49a072** / PR #215. **Update t_2ce05456 (tip-sync after #215):** Docs-only. Header/status product tip → **b49a072** / #215 (t_49452ed3). Keep **c55f8e1**/#212, **4553356**/#210, design #214/#207 in lineage. No product code.
**Update t_7e55fd9b (next product after polish design):** Docs-only. New [`docs/design-next-product-after-coding-agent-polish.md`](design-next-product-after-coding-agent-polish.md): inventory + GO/NO-GO; slice 1 `coding_agent` concern split; slice 2 `doctor/validate`. Follow-up 4 DONE. No product code. **Update t_e7ea2b4b (coding_agent concern-split slice 1):** Product. `coding_agent.gleam` 399→105 thin facade + types/select/classify/run; constructors via `coding_agent_types`; stable `grkr/coding_agent` fns; zero behavior change; design **cbf74fc** / #217. `gleam test` 347 green. Merge tip **20310e2** / PR #218. **Update t_68495d46 (tip-sync after #218):** Docs-only. Header/status product tip → **20310e2** / #218 (t_e7ea2b4b). Keep **b49a072**/#215, **c55f8e1**/#212, **4553356**/#210, design #217/#214/#207 in lineage. No product code. **Update t_74a7a161 (doctor/validate concern-split slice 2):** Product. `doctor/validate.gleam` 387→67 thin facade + tools 71 + agent 110 + config 132 + run 103; stable `grkr/doctor/validate`; zero behavior change; design **cbf74fc** / #217. `gleam test` 347 green. Merge tip **88e3eea** / PR #220. **Update t_355bbd31 (tip-sync after #220):** Docs-only. Header/status product tip → **88e3eea** / #220 (t_74a7a161). Keep **20310e2**/#218, **b49a072**/#215, design **cbf74fc**/#217 in lineage. No product code.
