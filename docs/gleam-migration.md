...
- Linear publish + complete @ **bd523a6** (PR #100) + guarded live mutate apply **landed** @ **e28d0c0** (PR #104) + post-land nits tip **8d4b674** (PR #107): after test, produces GitHub PR from linear-* + plans/applies (when enabled) Linear Done + completion comment. Guarded apply (`GRKR_LINEAR_MUTATE=1` literal enables; default OFF, soft-fail, *.linear-apply-result.txt sidecars, stage-scoped grkr-state-<stage>-<id> keys only, stricter `mutation_result` parse, soft resume-safe `skipped-no-token`, COMMENT FIRST then state for complete). No live in default tests/CI. Supervisor Linear pick→schedule→spawn already landed. GitHub remains default `GRKR_ISSUE_PROVIDER`.
