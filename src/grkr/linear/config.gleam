...
pub fn parse_oauth_credentials(content: String) -> Result(types.OAuthCredentials, String) {
  ...
  case get_field("client_id") {
    Error(err) -> Error(err)
    Ok(client_id) -> case get_field("client_secret") {
      Error(err) -> Error(err)
      Ok(client_secret) -> case client_id, client_secret {
  ...
  case enabled {
    ...
    True -> {
      case read_secret_file() {
        Error(err) -> Error(err)
        Ok(secret_content) -> case parse_oauth_credentials(secret_content) {
          Error(err) -> Error(err)
          Ok(creds) -> {
...
pub fn redact_config(config: types.E2EConfig) -> String {
  ...
  "Linear E2E: " <> enabled_str <> "\n"
  <> "Credentials: " <> redact_credentials(config.credentials) <> "\n"
  <> "Token: " <> token_status
}
