...
//// Full Gleam port of @robot command handling per spec/parts/15-phase-3-detect-and-process-robot-comments.md
//// GitHub-only v2. Thin bin/worker-handle-comment.sh delegates to workflow/handle_comment which uses this.
//// Implements: fetch context, eyes/rocket reactions, worktree (spec/12), codex classify (CLASS/REPLY/CHANGES),
//// result comment, optional commit/push, cleanup. Always best-effort exit 0.
...
fn emit_usage() {
  ...
  console_error("Full @:robot: comment handler (GitHub-only v2) per spec/15.")
  exit(2)
}
...
fn do_handle(comment_id: String) {
  case int.parse(comment_id) {
    Ok(_) -> Nil
    Error(_) -> {
      console_error("Error: comment_id must be numeric (GitHub id)")
      exit(1)
    }
  }

  let _ = console_log("🤖 grkr/supervisor/comment_handler: starting for comment_id=" <> comment_id <> " (GitHub-only v2 full)")
  ...
  case fetch_context(comment_id, repo, main_branch) {
    Ok(ctx) -> {
      let _ = console_log("   context: comment by @" <> ctx.user_login <> " on " <> case ctx.is_pr { True -> "PR " False -> "" } <> "#" <> ctx.issue_number <> " \"" <> ctx.issue_title <> "\" cmd=\"" <> ctx.raw_cmd <> "\"")
      ...
      let _ = console_log("   parsed: class=" <> class <> " changes=" <> changes)
      ...
      let _ = console_log("✅ comment_handler complete for " <> comment_id <> " (class=" <> class <> ", exit=0)")
      exit(0)
    }
    Error(e) -> {
      console_error("⚠️ comment_handler fetch failed (best-effort no-op): " <> e)
      exit(0)
    }
  }
}
    ...
    ExecResult(0, out, _) -> {
      case ffi.parse(out) {
        Ok(root) -> case ffi.get_field(root, "id") |> ffi.decode_string {
          Ok(id) -> {
            let _ = console_log("   + eyes reaction (id=" <> id <> ")")
            id
          }
          _ -> {
            let _ = console_log("   ⚠️ eyes reaction add skipped/failed (best effort)")
            ""
          }
        }
        _ -> ""
...
fn create_comment_worktree(comment_id: String, ctx: CommentContext, repo: String, main_branch: String) -> WorktreeInfo {
  ...
  let _ = console_log("   worktree: " <> case ctx.is_pr { True -> "PR comment" False -> "issue comment" } <> " base=" <> base_ref)
  ...
  case ffi.git_exec(add_args, None) {
    ExecResult(0, _, _) -> {
      let _ = console_log("   + worktree created at " <> worktree_dir <> " (branch " <> branch_name <> ")")
    ...
    _ -> {
      let _ = console_log("   ⚠️ worktree create failed; falling back to temp dir (no git ops)")
...
fn run_codex_classify(ctx: CommentContext, branch: String) -> String {
  ...
  let _ = console_log("   building codex prompt (len=" <> int.to_string(string.length(prompt)) <> ")")
  ...
  let _ = console_log("   codex raw output (truncated): " <> string.slice(out, 0, 300) <> "...")
  out
}
...
fn post_result_comment(ctx: CommentContext, comment_id: String, repo: String, class: String, reply: String, changes: String, branch: String) -> Nil {
  ...
  case run_gh(cmd) {
    ExecResult(0, _, _) -> console_log("   + posted result comment on #" <> ctx.issue_number)
    _ -> console_log("   ⚠️ failed to post result comment (best effort; continuing)")
  }
}
      ...
      case string.trim(out) {
        ...
        _ -> {
          ...
          case ffi.executable("git", ["-C", dir, "push", "--force-with-lease", "origin", branch], None) {
            ExecResult(0, _, _) -> console_log("   + pushed branch " <> branch <> " (code-change)")
            _ -> console_log("   ⚠️ push skipped (no perms or no changes)")
          }
        }
      }
    }
...
fn remove_eyes_and_add_rocket(comment_id: String, repo: String, eyes_id: String) -> Nil {
  ...
  case run_gh(rocket) {
    ExecResult(0, _, _) -> console_log("   + rocket reaction (success path)")
    _ -> console_log("   ⚠️ rocket reaction add failed (best effort)")
  }
}
